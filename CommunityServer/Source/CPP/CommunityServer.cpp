...
//		delete result;
	switch (signum)
	{
		case SIGINT:
		{
			Community::StopNow(SHUTDOWN_EXIT_CODE);
			break;
		}
		case SIGTERM:
		{
...
bool CommunityServer::loadDataTable()
{
	...
	*/
	...
	//fileNameList.SetFileName(TableContainer::TABLE_QUEST_REWARD,			"Table_Quest_Reward_Data");//structure need
	//fileNameList.SetFileName(TableContainer::TABLE_DRAGONBALL, "Table_Dragon_Ball_Data");//structure need
	fileNameList.SetFileName(TableContainer::TABLE_MERCHANT,				"Table_Merchant_Data");//structure need
	fileNameList.SetFileName(TableContainer::TABLE_HLS_ITEM,				"table_hls_item_data"); // structure need
...
bool CommunityServer::ConnectToDatabase()
{
...
int CommunityServer::Run()
{
	...
	if (sXmlParser.loadFile("CommunityServer") == false)
		return 1;
