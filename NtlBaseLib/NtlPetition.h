...
#include <vector>
//-- define -------------------------------------------------------------------------
#define MIN_SIZE_QUESTION_CONTENT							(4)
#define MAX_SIZE_QUESTION_CONTENT							(256)
#define MAX_SIZE_QUESTION_CONTENT_UNICODE					(256)
#define MAX_SIZE_QUESTION_CONTENT_BUFFER_MULTIBYTE			(512)

#define MIN_SIZE_CUSTOMER_SATISFACTION						(4)
#define MAX_SIZE_CUSTOMER_SATISFACTION						(512)
#define MAX_SIZE_CUSTOMER_SATISFACTION_UNICODE				(512)
#define MAX_SIZE_CUSTOMER_SATISFACTION_BUFFER_MULTIBYTE		(1024)

#define MIN_SIZE_PETITION_RESULTTEXT						(4)
#define MAX_SIZE_PETITION_RESULTTEXT						(512)
#define MAX_SIZE_PETITION_RESULTTEXT_UNICODE				(512)
#define MAX_SIZE_PETITION_RESULTTEXT_BUFFER_MULTIBYTE		(1024)

#define MIN_SIZE_NOTE										(0)
#define MAX_SIZE_NOTE										(512)
#define MAX_SIZE_NOTE_UNICODE								(512)
#define MAX_SIZE_NOTE_BUFFER_MULTIBYTE						(1024)

#define MAX_PETITION_GETLIST_COUNT							(20)

#define MAX_SIZE_RESERVEGMID								(256)//보류 GM ID
#define MAX_SIZE_RESERVEGMID_UNICODE						(256)//보류 GM ID
#define MAX_SIZE_RESERVEGMID_BUFFER_MULTIBYTE				(512)

#define MAX_SIZE_STARTGMID									(256)// 대응 GM ID
#define MAX_SIZE_STARTGMID_UNICODE							(256)// 대응 GM ID
#define MAX_SIZE_STARTGMID_BUFFER_MULTIBYTE					(512)

#define MAX_SIZE_CHANGECATEGORYGMID							(256)// 전달 GM ID
#define MAX_SIZE_CHANGECATEGORYGMID_UNICODE					(256)// 전달 GM ID
#define MAX_SIZE_CHANGECATEGORYGMID_BUFFER_MULTIBYTE		(512)

#define MAX_SIZE_LOG_TEXT									(1024)
#define MAX_SIZE_LOG_TEXT_UNICODE							(1024)
#define MAX_SIZE_LOG_TEXT_BUFFER_MULTIBYTE					(2048)

#define MAX_SIZE_PETITIONCOUNSEL_USERID						( MAX_SIZE_USERID + MAX_SIZE_CHAR_NAME )
#define MAX_SIZE_PETITIONCOUNSEL_USERID_UNICODE				( MAX_SIZE_USERID_UNICODE + MAX_SIZE_CHAR_NAME_UNICODE )
#define MAX_SIZE_PETITIONCOUNSEL_USERID_BUFFER_MULTIBYTE	( MAX_SIZE_USERID_MULTIBYTE_BUFFER + MAX_SIZE_CHAR_NAME_BUFFER_MULTIBYTE )
...
struct sPETITION_PROGRESSSTATE_DATA
{
	...
	WCHAR							awchGMUserId[MAX_SIZE_USERID_UNICODE  + 1];
	WCHAR							awchPetitionCharName[MAX_SIZE_CHAR_NAME_UNICODE + 1];
	...
	union uDATA
	{
		...
		struct sRESERVE
		{
			WCHAR		awchNote[MAX_SIZE_NOTE_UNICODE + 1];//비고
		} sReserve;
		...
		struct sFINISH
		{
			...
			ePETITIONSYSTEM_CATEGORY_1	eNewCategory1;
			ePETITIONSYSTEM_CATEGORY_2	eNewCategory2;
			WCHAR						awchPetitionResultText[MAX_SIZE_PETITION_RESULTTEXT_UNICODE + 1];
		} sFinish;
		...
		struct sMAIL
		{
			...
			BYTE  						byTextSize;
			WCHAR						wszText[MAX_LENGTH_OF_MAIL_MESSAGE_IN_UNICODE + 1];// 메일내용
		} sMail;
		...
		struct sSAY // Start, End, GM Say, User Say
		{
			...
			WCHAR				wszPetitionSay[MAX_LENGTH_OF_CHAT_MESSAGE_UNICODE + 1];
		} sSay;
...
struct sPETITIONLISTDATA
{
	...
	WCHAR									awchUserCharacterName_PetitionCreate[MAX_SIZE_CHAR_NAME_UNICODE + 1];
	WCHAR									awchGMCharacterName_PetitionCreate[MAX_SIZE_CHAR_NAME_UNICODE + 1]; // 없애자
...
struct sPETITIONDATA
{
	...
	WCHAR									awchUserAccount_PetitionCreate[MAX_SIZE_USERID_UNICODE + 1];// Petition 생성당시 유져의 Account ID ( 무조건 채워진다 )
	WCHAR									awchUserCharacterName_PetitionCreate[MAX_SIZE_CHAR_NAME_UNICODE + 1];// Petition 생성당시 유져의 Char ID ( 무조건 채워진다 )
	WCHAR									awchGMAccount_PetitionCreate[MAX_SIZE_USERID_UNICODE + 1];// Petition을 생성한 GM의 Account ID ( 유져 생성시 "" )
	WCHAR									awchGMCharacterName_PetitionCreate[MAX_SIZE_CHAR_NAME_UNICODE + 1];// Petition을 생성한 GM의 Account ID ( 유져 생성시 "" )

	WCHAR									awchQuestionContent[MAX_SIZE_QUESTION_CONTENT_UNICODE + 1]; // 문의 내용
	eCUSTOMERSATISFACTION					eCustomerSatisfaction; // 고객 만족도
	WCHAR									awchPetitionResultText[MAX_SIZE_PETITION_RESULTTEXT_UNICODE + 1]; // 상담 결과
	WCHAR									awchNote[MAX_SIZE_NOTE_UNICODE + 1]; // 비고 ( 보류할때 쓰는 비고 )
	...
	WCHAR									awchBlackList[ MAX_LENGTH_OF_CHAT_MESSAGE_UNICODE + 1];

	//bool									bIsConnectUser;
	//SERVERFARMID							serverfarmID_ConnectUser;
	//WCHAR									awchUserCharacterName_ConnectUser[MAX_SIZE_CHAR_NAME_UNICODE + 1];
	//CHARACTERID							characterID_ConnectUser;

	GMACCOUNTID								gmAccountID_Consult; // 대응 정보의 GM Account ID( 시리얼 )
	WCHAR									awchGMAccount_Consult[MAX_SIZE_USERID_UNICODE + 1]; // 대응 상황의 GM Account ( Char )
	WCHAR									awchGMCharName_Consult[MAX_SIZE_CHAR_NAME_UNICODE + 1];// 대응 상황의 GM 이름

	WCHAR									awchReserveGMID[MAX_SIZE_RESERVEGMID_UNICODE + 1]; //대응 상황의 보류 GM ID
	WCHAR									awchStartGMID[MAX_SIZE_STARTGMID_UNICODE + 1]; // 대응 상황의 대응 GM ID
	WCHAR									awchChangeCategoryGMID[MAX_SIZE_CHANGECATEGORYGMID_UNICODE + 1]; // 대응상황의 카테고리 변경 GM ID
...
struct sPETITION_COUNSELDATA
{
	...
	WCHAR									awchAccount[MAX_SIZE_PETITIONCOUNSEL_USERID_UNICODE + 1];
	WCHAR									awchText[MAX_SIZE_LOG_TEXT_UNICODE + 1];
