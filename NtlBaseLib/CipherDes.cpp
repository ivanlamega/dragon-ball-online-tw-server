...
void CipherDes::permute(char *inblock, char perm[16][16][8], char *outblock)
{
	REGISTER int i, j;
...
void CipherDes::perm32(char *inblock, char *outblock)               /* 32-bit permutation at end    */
{
	REGISTER int j;
...
void CipherDes::expand(char *right, char *bigright)                 /* 32 to 48 bits with E oper    */
{
...
void CipherDes::contract(char *in48, char *out32)                   /* contract f from 48 to 32 bits*/
{
	REGISTER char *c;
	REGISTER char *i;
...
void CipherDes::f(char *right, int num, char *fret)                 /* critical cryptographic trans */
{
...
void CipherDes::iter(int num, char *inblock, char *outblock)        /* 1 churning operation         */
{
...
int CipherDes::getcomp(int k, int v)                                /* 1 compression value for sinit*/
{
...
void CipherDes::kinit(char *key64bit)       /* initialize key schedule array*/
{
	REGISTER int i, j, l;
	int m;
...
//    char p[64];
void CipherDes::perminit(char perm[16][16][8], char p[64])                  /* initialize a perm array      */
{
	REGISTER int l, j, k;
	int i, m;
...
void CipherDes::endes(char *inblock, char *outblock)                /* encrypt 64-bit inblock       */
{
...
void CipherDes::dedes(char *inblock, char *outblock)                /* decrypt 64-bit inblock       */
{
	char iters[17][8];                  /* workspace for each iteration */
	char swap[8];                       /* place to interchange L and R */
	REGISTER int i;
...
void CipherDes::DesWriteBlock(void *buf, int len)
{
	char *outString;
...
void CipherDes::DesReadBlock(void *buf, int len)
{
	char *outString;
	int mLen = len & 0xfffffff8;
	for (outString = (char*)buf; mLen > 0; mLen -= 8, outString += 8)
...
void CipherDes::DesKeyInit(char *keyValue)
...
int CipherDes::SetKey(unsigned char *cipherKey, int cipherKeyLength)
{
...
int CipherDes::Encrypt(char *originalData, int originalDataLength, char*encData, int encDataSize)
{
	int retLength = 0;
	int blockLen = 0;
...
int CipherDes::Decrypt(char *originalData, int originalDataLength, char *decData, int decDataSize)
{
	if (!originalData)
		return NULL_INPUTDATA;
	if (!decData)
