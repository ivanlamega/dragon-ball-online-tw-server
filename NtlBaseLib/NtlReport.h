...
struct sGM_REPORT_CONTENT
{
	...
	char				accountID[MAX_SIZE_USERID + 1];
	char				targetID[MAX_SIZE_USERID + 1];
	char				targetName[MAX_SIZE_GUILD_NAME + 1]; // 가장 긴 타겟은 유파명
	char				beforeAction[255]; // 따로 define 할 예정
	char				afterAction[255];
	char				objectName[MAX_LENGTH_REPORT_NAME + 1];
	TBLIDX				tblIdx;
	int					serial;
	int					result;
};
