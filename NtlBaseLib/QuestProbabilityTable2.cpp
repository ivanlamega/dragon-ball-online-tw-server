...
WCHAR* QuestProbabilityTable2::m_pwszSheetList[] =
{
	L"Table_Data_KOR",
	NULL
};

QuestProbabilityTable2::QuestProbabilityTable2(void)
{
	Init();
}
...
bool QuestProbabilityTable2::Create(DWORD dwCodePage)
{
	return Table::Create(dwCodePage);
}

void QuestProbabilityTable2::Destroy()
{
	Table::Destroy();
}

void QuestProbabilityTable2::Init()
{
}

void* QuestProbabilityTable2::AllocNewTable(WCHAR* pwszSheetName, DWORD dwCodePage)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
		sQUEST_PROBABILITY_TBLDAT2 * pQuestProbability = new sQUEST_PROBABILITY_TBLDAT2;
...
bool QuestProbabilityTable2::DeallocNewTable(void* pvTable, WCHAR* pwszSheetName)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
		sQUEST_PROBABILITY_TBLDAT2 * pQuestProbability = (sQUEST_PROBABILITY_TBLDAT2 *)pvTable;
...
bool QuestProbabilityTable2::AddTable(void * pvTable, bool bReload)
{
	UNREFERENCED_PARAMETER(bReload);

	sQUEST_PROBABILITY_TBLDAT2 * pTbldat = (sQUEST_PROBABILITY_TBLDAT2 *)pvTable;


	for (BYTE bySlot = 0; bySlot < QUEST_PROBABILITY_MAX_COUNT; bySlot++)
	{
...
bool QuestProbabilityTable2::SetTableData(void* pvTable, WCHAR* pwszSheetName, std::wstring* pstrDataName, BSTR bstrData)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
		sQUEST_PROBABILITY_TBLDAT2 * pProbabilityTbldat = (sQUEST_PROBABILITY_TBLDAT2*)pvTable;
		...
		else
		{
			...
			// Reward Type
			for (int i = 0; i < QUEST_PROBABILITY_MAX_COUNT; i++)
			{
...
sTBLDAT* QuestProbabilityTable2::FindData(TBLIDX tblidx)
{
	if (0 == tblidx)
		return NULL;
...
bool QuestProbabilityTable2::LoadFromBinary(Serializer& serializer, bool bReload)
{
	if (false == bReload)
	{
		Reset();
	...
	do
	{
		sQUEST_PROBABILITY_TBLDAT2* pTableData = new sQUEST_PROBABILITY_TBLDAT2;
		if (NULL == pTableData)
		{
			// Log : [11/29/2007 niam]
...
bool QuestProbabilityTable2::SaveToBinary(Serializer& serializer)
{
	serializer.Refresh();
	...
	{
		sQUEST_PROBABILITY_TBLDAT2* pTableData = (sQUEST_PROBABILITY_TBLDAT2*)(iter->second);
