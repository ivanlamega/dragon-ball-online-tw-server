...
{
	ROOMID	roomId;
	WCHAR	wszOwnerPc[MAX_SIZE_CHAR_NAME_UNICODE + 1];	// 주인
	WORD	wWinCounter;
};
...
{
	ROOMID	roomId;
	WCHAR	wszOwnerParty[MAX_SIZE_PARTY_NAME_IN_UNICODE + 1];	// 주인
	WORD	wWinCounter;
};
...
struct sRANKBATTLE_ROOMINFO_PARTY
{
	...
	sRANKBATTLE_ROOMINFO_MEMBER	asMember[MAX_MEMBER_IN_PARTY];		// Member
};
...
{
	WCHAR				wszCharName[MAX_SIZE_CHAR_NAME_UNICODE];
	BYTE				byLevel;
	BYTE				byClass;
	WCHAR				wszGuildName[MAX_SIZE_GUILD_NAME_IN_UNICODE];
	int					nPoint;
...
{
	WORD				wCharName;				// WCHAR [MAX_SIZE_CHAR_NAME_UNICODE]
	BYTE				byLevel;
	BYTE				byClass;
	WORD				wGuildName;				// WCHAR [MAX_SIZE_GUILD_NAME_IN_UNICODE]
	int					nPoint;
	WORD				wStraightWinCount;
	DWORD				dwCompareRank;
};
