...
const DWORD		MAX_BASIC_MAIL_SEND_ZENNY	= 10;		// 수수료
const DWORD		MAX_ATTACH_MAIL_SEND_ZENNY	= 10;		// 수수료
const DWORD		MAX_MAIL_SEND_ZENNY_AMOUNTS	= 100000;	// 첨부 최대가능 금액
const DWORD		MAX_MAIL_SLOT_COUNT	= 30;				// 최대 메일 슬롯의 개수
const DWORD		MAIL_RELOAD_TIME    = 1 * 60 * 1000;    // DB 내용과 동기화 시간 조정 필요
const DWORD		MAIL_DISTANCE_CHECK_TIME = 3 * 1000;	// 메일박스와의 거리 체크 시간 
const BYTE		DEFAULT_MAIL_KEEPING_PERIOD = 10;		// 기본 메일 유지 기간. 일(day) 단위
const DWORD		MAX_COUNT_OF_MULTI_DEL		= 6;		// 멀티델가능 최대수
const BYTE		MAX_COUNT_MULTI_MAIL		= 50;		// GM-TOOL 멀티메일 최대치
//-----------------------------------------------------------------------------------
// 메일 타입 
//-----------------------------------------------------------------------------------
enum eMAIL_TYPE
...
struct sMAIL_SLOT
{
	MAILID aMailID[MAX_MAIL_SLOT_COUNT];
};
...
struct sMAIL_DATA
{
	...
	WCHAR				wszFromName[MAX_SIZE_CHAR_NAME_UNICODE + 1];		// 보낸 캐릭 이름
	BYTE				byTextSize;
