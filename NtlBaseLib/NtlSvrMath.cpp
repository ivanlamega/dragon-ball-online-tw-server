...
bool CNtlSvrMath::Line2DBBox2DTest( const CNtlLine& rLine, const CNtlBBox& rBBox )
{
	...
	else
	{
		...
		float fLhs = static_cast<float>(abs( (int)rPerp.Dot( rDiff ) ));
		float fPart0 = static_cast<float>(abs((int)rPerp.Dot( rBBoxVal.m_rXAxis ) ));
		float fPart1 = static_cast<float>(abs((int)rPerp.Dot( rBBoxVal.m_rZAxis ) ));
