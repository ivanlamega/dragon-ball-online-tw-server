...
WCHAR* HTBSetTable::m_pwszSheetList[] =
{
	L"Table_Data_KOR",
	NULL
};

HTBSetTable::HTBSetTable(void)
{
	Init();
}
...
bool HTBSetTable::Create(DWORD dwCodePage)
{
	return Table::Create(dwCodePage);
}

void HTBSetTable::Destroy()
{
	Table::Destroy();
}

void HTBSetTable::Init()
{
}

void* HTBSetTable::AllocNewTable(WCHAR* pwszSheetName, DWORD dwCodePage)
{
...
bool HTBSetTable::DeallocNewTable(void* pvTable, WCHAR* pwszSheetName)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
...
bool HTBSetTable::AddTable(void * pvTable, bool bReload)
{
...
bool HTBSetTable::SetTableData(void* pvTable, WCHAR* pwszSheetName, std::wstring* pstrDataName, BSTR bstrData)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
		sHTB_SET_TBLDAT * pHTBSet = (sHTB_SET_TBLDAT*)pvTable;
		...
		else if (0 == wcsncmp(pstrDataName->c_str(), L"HTB_Type_", wcslen(L"HTB_Type_")))
		{
			...
			for (int i = 0; i < HTB_MAX_SKILL_COUNT_IN_SET; i++)
			{
...
sTBLDAT* HTBSetTable::FindData(TBLIDX tblidx)
{
	if (0 == tblidx)
		return NULL;
...
bool HTBSetTable::LoadFromBinary(Serializer& serializer, bool bReload)
{
	if (false == bReload)
	{
		Reset();
...
bool HTBSetTable::SaveToBinary(Serializer& serializer)
{
	serializer.Refresh();
