...
enum eBATTLE_ATTACK_RESULT
{
	...
	BATTLE_ATTACK_RESULT_DODGE, // evasion
	BATTLE_ATTACK_RESULT_RESISTED, // resistance
	BATTLE_ATTACK_RESULT_BLOCK, // Block
	BATTLE_ATTACK_RESULT_REFLECTED_DAMAGE,// damage that is reflected from the target
	BATTLE_ATTACK_RESULT_KNOCKDOWN, // knock down
	BATTLE_ATTACK_RESULT_SLIDING,// sliding
	BATTLE_ATTACL_RESULT_IMMUNE,
...
// 공격 결과
enum eBATTLE_SPECIAL_ATTACK_RESULT
{
	BATTLE_SPECIAL_ATTACK_RESULT_HIT, // General attack
	BATTLE_SPECIAL_ATTACK_RESULT_BLOCK, // Block
	BATTLE_SPECIAL_ATTACK_RESULT_DODGE, // evasion
	BATTLE_SPECIAL_ATTACK_RESULT_REFLECTED_DAMAGE,// damage that is reflected from the target
	BATTLE_SPECIAL_ATTACK_RESULT_KNOCKDOWN, // knock down
	BATTLE_SPECIAL_ATTACK_RESULT_HEALT,

	BATTLE_SPECIAL_ATTACK_RESULT_SLIDING,// sliding
	BATTLE_SPECIAL_ATTACK_RESULT_NOTHING,
	BATTLE_SPECIAL_ATTACL_RESULT_IMMUNE,
	BATTLE_SPECIAL_ATTACK_RESULT_RESISTED, // resistance

	BATTLE_SPECIAL_ATTACK_RESULT_COUNT,

	BATTLE_SPECIAL_ATTACK_RESULT_FIRST = BATTLE_SPECIAL_ATTACK_RESULT_HIT,
	BATTLE_SPECIAL_ATTACK_RESULT_LAST = BATTLE_SPECIAL_ATTACK_RESULT_RESISTED
};
...
//-----------------------------------------------------------------------------------
const float			BATTLE_PUSH_DISTANCE					= 1.0f; // PUSH 거리
const float			BATTLE_KNOCKDOWN_DISTANCE				= 5.6f;	// KNOCKDOWN 거리.
const float			BATTLE_SLIDING_DISTANCE					= 4.0f; // SLIDING 거리

const int			BATTLE_CHAIN_ATTACK_START				= 1; // chain attack start id 

const int			BATTLE_MAX_CHAIN_ATTACK_COUNT			= 6; // 최대 체인 어택 값

const float			BATTLE_BLOCK_DAMAGE_REDUCE_RATE			= 0.3f;
const float			DBO_BATTLE_OFFENCE_CHANGE_RATE_BY_BLOCK			= 0.3f;
const float			BATTLE_CRITICAL_DAMAGE_INCREASE_RATE	= 1.5f;
const float			DBO_BATTLE_OFFENCE_BONUS_RATE_BY_CRITICAL		= 50.0f;		// in percent

const float			BATTLE_HIT_FRONT_SIDE_BONUS_RATE		= 1.0f;
const float			BATTLE_HIT_BACK_SIDE_BONUS_RATE			= 0.5f;

const DWORD			BATTLE_ATTACK_DELAY_TIME				= 1000;

const DWORD			BATTLE_KNOCKDOWN_WAKEUP_TIME			= 4000; // 넉다운시 일어나는 타임 ( NPC / MOB 용 )
const DWORD			BATTLE_KNOCKDOWN_MIN_WAKEUP_TIME		= 2000; // 넉다운시 일어나는 타임 ( PC용 )
const DWORD			BATTLE_KNOCKDOWN_MAX_WAKEUP_TIME		= 6000; // 넉다운시 일어나는 타임 ( PC용 )
const DWORD			BATTLE_KNOCKDOWN_WAKEUP_TIME_DIFF		= BATTLE_KNOCKDOWN_MAX_WAKEUP_TIME - BATTLE_KNOCKDOWN_MIN_WAKEUP_TIME;
const DWORD			BATTLE_SLIDING_END_TIME					= 1200; // 슬라이딩 끝나는 타임
const DWORD			BATTLE_HTB_SANDBAG_END_TIME				= 10000; // HTB시 SANDBAG 끝나는 타임

const int			BATTLE_MAX_NPC_ATTACK_TYPE				= 2; // NPC 의 공격 형태 ( 현재는 2가지 )
