...
// 천하제일무도회 본선 최대 연장전 카운트 주의 : MAX_MATCH_EXTENDED_STAGE_COUNT 보다 큰 숫자가 올 수 없다.
...
struct sMATCH_MINORMATCH_TEAM_INFO
{
	...
	BYTE						byMemberCount;
	sMATCH_MEMBER_STATE_INFO	aMembers[MAX_MEMBER_IN_PARTY];
};
...
{
	TEAMTYPE	wTeamType;
	WORD		wTeamName_var;		//	WCHAR [MAX_LENGTH_BUDOKAI_TEAM_NAME_IN_UNICODE + 1];
	bool		bDojoRecommender;	// 도장 추천
	BYTE		byMemberCount;
	WORD		wMembers_var;		// sMATCH_MEMBER_STATE_INFO [MAX_MEMBER_IN_PARTY];
};
