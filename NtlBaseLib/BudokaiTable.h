...
#define BUDOKAI_MAX_MAJOR_LOCATION_COUNT (MAX_MATCH_TEAM_TYPE_COUNT * MAX_MEMBER_IN_PARTY)
#define BUDOKAI_MAX_FINAL_LOCATION_COUNT (MAX_MATCH_TEAM_TYPE_COUNT * MAX_MEMBER_IN_PARTY)
...
struct sMINORMATCH_REWARD
{
	...
	BYTE	byLoserItemStackCount;		// 패자 아이 스택카운트
};
...
class BudokaiTable : public Table
{
public:
	BudokaiTable(void);
	virtual ~BudokaiTable(void);
...
protected:
	...
	bool GetBinaryText(std::wstring & wstrValue, Serializer& serializer);
