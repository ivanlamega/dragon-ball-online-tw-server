...
bool NewbieTable::SetTableData(void* pvTable, WCHAR* pwszSheetName, std::wstring* pstrDataName, BSTR bstrData)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
		...
		else if (0 == wcscmp(pstrDataName->c_str(), L"Default_Portal_Tblidx1"))
		{
			pNewbie->defaultPortalTblidx1 = READ_BYTE(bstrData, pstrDataName->c_str());
		}
		else if (0 == wcscmp(pstrDataName->c_str(), L"Default_Portal_Tblidx2"))
		{
			pNewbie->defaultPortalTblidx2 = READ_BYTE(bstrData, pstrDataName->c_str());
		}
