...
WCHAR* WorldTable::m_pwszSheetList[] =
{
	L"Table_Data_KOR",
	//L"Table_Data_TW",
	NULL
...
bool WorldTable::Create(DWORD dwCodePage)
{
	return Table::Create(dwCodePage);
}

void WorldTable::Destroy()
{
	Table::Destroy();
}

void WorldTable::Init()
{
}

void* WorldTable::AllocNewTable(WCHAR* pwszSheetName, DWORD dwCodePage)
{
...
bool WorldTable::DeallocNewTable(void* pvTable, WCHAR* pwszSheetName)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
...
bool WorldTable::AddTable(void * pvTable, bool bReload)
{
...
bool WorldTable::SetTableData(void* pvTable, WCHAR* pwszSheetName, std::wstring* pstrDataName, BSTR bstrData)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
		sWORLD_TBLDAT* pWorld = (sWORLD_TBLDAT*)pvTable;
...
sTBLDAT* WorldTable::FindData(TBLIDX tblidx)
{
	if (0 == tblidx)
		return NULL;
...
bool WorldTable::LoadFromBinary(Serializer& serializer, bool bReload)
{
	...
	unsigned char dataSize = static_cast<unsigned char>(sizeof(sWORLD_TBLDAT));
...
bool WorldTable::SaveToBinary(Serializer& serializer)
{
	serializer.Refresh();
