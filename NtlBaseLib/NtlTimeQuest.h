...
const DWORD		DBO_TIMEQUEST_MAX_MEMBER_COUNT = MAX_MEMBER_IN_PARTY; // 타임머신 최대 출발 인원
...
{
	HOBJECT					handle;
	WCHAR					awchName[ MAX_SIZE_CHAR_NAME_UNICODE + 1 ]; // 캐릭터 이름
...
{
	WCHAR					awchName[ MAX_SIZE_PARTY_NAME_IN_UNICODE + 1 ];
};
...
struct sTIMEQUEST_DAYRECORD_DATA
{
	...
	DWORD		dwClearTime;
	BYTE		byMemberCount;
	CHARACTERID	aMemberCharId[MAX_MEMBER_IN_PARTY];
};
...
struct sTIMEQUEST_TEAM_DATA
{
	...
	WCHAR				wszPartyName[MAX_SIZE_PARTY_NAME_IN_UNICODE];
	BYTE				byMemberCount;
	WCHAR				awszMember[MAX_MEMBER_IN_PARTY][MAX_SIZE_CHAR_NAME_UNICODE + 1];
};
...
{
	BYTE							byMemberCount;
	sTIMEQUEST_PARTY_MEMBER_DATA	asMember[MAX_MEMBER_IN_PARTY];
};
...
{
	WORD		wName;		// WCHAR[MAX_SIZE_CHAR_NAME_UNICODE]
	CHARACTERID	charId;
};
...
{
	DWORD				dwClearTime;	// clear time
	WORD				wPartyName;		// WCHAR [MAX_SIZE_PARTY_NAME_IN_UNICODE];
	BYTE				byMemberCount;
	WORD				wMember;		// sTIMEQUEST_MEMBER [MAX_MEMBER_IN_PARTY];
};
