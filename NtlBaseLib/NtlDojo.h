...
public:
	...
	WCHAR			wszName[MAX_SIZE_GUILD_NAME_IN_UNICODE + 1];	// 길드명
	GUILDID			challengeGuildId;									//	도전길드id
	WCHAR			wszSeedCharName[MAX_SIZE_CHAR_NAME_UNICODE + 1];		// 시드배정캐릭명

	WCHAR			wchLeaderName[MAX_SIZE_CHAR_NAME_UNICODE + 1];	// ''	공지자
	WCHAR			wchNotice[MAX_LENGTH_OF_DOJO_NOTICE_UNICODE + 1];	//'' 공지	
};
...
struct sDBO_DOJO_NPC_INFO
{
	...
	WCHAR				wszName[MAX_SIZE_GUILD_NAME_IN_UNICODE + 1];// 유파이름
	BYTE				byLevel;
	DWORD				dwGuildReputation;		// 길드명성
	DWORD				dwMaxGuildPointEver;	// Load 그동안 dwGuildReputation 가장 최고값
	BYTE				byPeaceStatus;	
	WCHAR				wszSeedCharName[MAX_SIZE_CHAR_NAME_UNICODE + 1];		// 시드배정캐릭명
	WCHAR				wchLeaderName[MAX_SIZE_CHAR_NAME_UNICODE + 1];		// 리더 캐릭명
	WCHAR				wchNotice[MAX_LENGTH_OF_DOJO_NOTICE_UNICODE + 1];
	WCHAR				wszChallengeName[MAX_SIZE_GUILD_NAME_IN_UNICODE + 1];
	WCHAR				wszChallengeLeaderName[MAX_SIZE_CHAR_NAME_UNICODE + 1];			// 도전유파장
};
