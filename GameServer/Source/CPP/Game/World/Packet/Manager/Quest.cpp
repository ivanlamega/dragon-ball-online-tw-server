	...
	for (int i = 0; i < contStart->GetNumOfChildEntity(); i++)
	{
		...
		switch (contStart->GetChildEntity(i)->GetEntityType())
		{
			...
			case DBO_EVENT_TYPE_ID_CLICK_NPC:
			{
				...
				// Tutorial -----------------
				if (clickNpc->GetNPCIdx() == 7611104)
				{
					SendDirectionIndicateNfy(false, 0, 0, 0);
				}
				// Tutorial -----------------
				break;
			}
			...
			case DBO_EVENT_TYPE_ID_RCV_SVR_EVT:
			{
				...
				if (rcvSvrEvt)
				{
					...
					else if (rcvSvrEvt->GetEvtID() == 16290)
					{
						QuestData* quest = _player->GetQuestManager()->FindQuestById(tid);
						if (quest)
						{
							quest->tlq1Info.handleNpc = _player->GetQuestManager()->handleNpcTlq1Temp;
							sLog.outBasic("Add handle %d npc to quest %d", quest->tlq1Info.handleNpc, quest->QuestID);
						}
					}
					// TLQ1 -----------------------------
					// TLQ2 -----------------------------
					else if (rcvSvrEvt->GetEvtID() == 16600)
					{
						SendCharDirectPlay(true, 1, 1032);
					...
					if (_player->GetWorldTableID() == colRgn->GetWorldIdx())
					{
						...
						{ 
							...
							Timer.setInterval([&](TimerArguments<CDboTSColRgn> args) {
									...
									{
										SendCharDirectPlay(true, 1, 1037);
			...
			case DBO_COND_TYPE_ID_CHECK_PROG_QUEST:
			{
				...
				if (checkQuest)
				{
					if (checkQuest->GetQuestID() == 11603)
					{
						QuestData* prevQuest = _player->GetQuestManager()->FindQuestById(checkQuest->GetQuestID());
						if (prevQuest)
						{
							if (prevQuest->tlq1Info.handleNpc)
							{
								QuestData* quest = _player->GetQuestManager()->FindQuestById(tid);
								if (quest)
								{
									quest->tlq1Info.handleNpc = prevQuest->tlq1Info.handleNpc;
									sLog.outBasic("NPC Handle %d cur quest %d prev quest %d", prevQuest->tlq1Info.handleNpc, quest->QuestID, prevQuest->QuestID);
								}
							}
						}
					}
		...
		switch (contGAct->GetChildEntity(i)->GetEntityType())
		{
			...
			case DBO_ACT_TYPE_ID_ACT_REGQINFO:
			{
				...
				if (tid == 11617 && tcId == 32)
				{
					...
					Timer.setInterval([&](int idTimer2) {
						if (_player != NULL)
						{
							float distance = NtlGetDistance(_player->m_position.x, _player->m_position.z, 137.2, -199.17999);
							sLog.outDebug("Distance %f", distance);
							if (distance < 100.0f)
							{
								SendCharDirectPlay(true, 1, 1036);
								Timer.stop(idTimer2);

								// Add mobs with setTimeout
								int idTimer3 = Timer.GetNewId();
								Timer.setTimeout([&]() {
									CreateMobDino1TLQ2();
									}, 24000, idTimer3);
							}
			...
			case DBO_ACT_TYPE_ID_ACT_SEND_SVR_EVT:
			{
				...
				if (sendSvrEvt)
				{
					...
					else if (sendSvrEvt->GetSvrEvtID() == 1100601)
					{
						HOBJECT handleNpc = CreateNPCTurtleTutorial();
						Npc* npcInfo = static_cast<Npc*>(_player->GetFromList(handleNpc));
						if (npcInfo)
						{
							sVECTOR3 npcPos;
							npcInfo->GetPosition(npcPos.x, npcPos.y, npcPos.z);
							SendDirectionIndicateNfy(true, npcPos.x, npcPos.y, npcPos.z);
						}
					}
					else if (sendSvrEvt->GetSvrEvtID() == 1100801)
					{
						HOBJECT handleNpc = CreateNPCTrunksTutorial();
						Npc* npcInfo = static_cast<Npc*>(_player->GetFromList(handleNpc));
						if (npcInfo)
						{
							sVECTOR3 npcPos;
					...
					{
						HOBJECT handleNpc = CreateNPCRobotTutorial();
						Npc* npcInfo = static_cast<Npc*>(_player->GetFromList(handleNpc));
						if (npcInfo)
						{
							sVECTOR3 npcPos;
					...
					{
						ConvertGohanMobNPC(7451107, tid);
					}
					...
					{
						SendCharDirectPlay(true, 1, 1038);
					...
					{
						SendCharDirectPlay(true, 1, 1031);
					...
					{
						TBLIDX idMobNpc1 = 4131101;
						TBLIDX idMobNpc2 = 4132101;
						...
						if (quest)
						{
							...
							quest->tlq3Info.maxKillMob = 2;
							quest->tlq3Info.curKillMob = 0;

							ConvertNPC1MobNPC(idMobNpc1, quest->QuestID);
							ConvertNPC2MobNPC(idMobNpc2, quest->QuestID);
					...
					else if (sendSvrEvt->GetSvrEvtID() == 16780)
					{
						...
						int idTimer = Timer.GetNewId();
						Timer.setTimeout([&]() {
							SendTSUpdateEventNfy(TS_TYPE_QUEST_CS, 16790);
							CreateNPCOolongTLQ3();
						}, 12000, idTimer);
					}
					else if (sendSvrEvt->GetSvrEvtID() == 16850)
					{
						HOBJECT handleNpc1 = CreateNPCNpc1TLQ3();
						HOBJECT handleNpc2 = CreateNPCNpc2TLQ3();

						_player->GetQuestManager()->AddNPCSpawnedQuest(handleNpc1, tid);
						_player->GetQuestManager()->AddNPCSpawnedQuest(handleNpc2, tid);
						...
						if (quest)
						{
							quest->tlq3Info.handleNpc1 = handleNpc1;
							quest->tlq3Info.handleNpc2 = handleNpc2;
							sLog.outDetail("NPCs HANDLE (%d, %d) ADD TO QUEST %d", handleNpc1, handleNpc2, quest->QuestID);
						}
					}
					else if (sendSvrEvt->GetSvrEvtID() == 16855)
					{
						SendCharDirectPlay(true, 1, 1042);

						int idTimer = Timer.GetNewId();

						Timer.setTimeout([&]() {
							SendTSUpdateEventNfy(TS_TYPE_QUEST_CS, 16856);
							}, 10000, idTimer);
					}
					else if (sendSvrEvt->GetSvrEvtID() == 16860 && tcId == 11)
					{
						ConvertNPC1MobNPC2(4131101, tid);
						ConvertNPC2MobNPC2(4132101, tid);
						ConvertNPC3MobNPC(1591101, tid);

						_player->GetQuestManager()->AddMobQuest(4131101, tid);
						_player->GetQuestManager()->AddMobQuest(4132101, tid);
						_player->GetQuestManager()->AddMobQuest(1591101, tid);

						QuestData* quest = _player->GetQuestManager()->FindQuestById(tid);
						if (quest)
						{
							quest->tlq3Info.maxKillMob = 3;
							quest->tlq3Info.curKillMob = 0;
						}
					}
					else if (sendSvrEvt->GetSvrEvtID() == 16980)
					{
						/*5551105  picolo
						7042101 tenshinhan
						2761801 goku
						...
						9042101 yamcha*/	
						CreateNPCPicoloTLQ3();
						CreateNPCTenshinhanTLQ3();
						CreateNPCGoku2TLQ3();
						CreateNPCKrillinTLQ3();
						CreateNPCChaozTLQ3();
						CreateNPCYamchaTLQ3();
						CreateNPCMilk2TLQ3();

						int idTimer = Timer.GetNewId();
						Timer.setInterval([&](int idTimer) {
							try
							{
								// Distance to goku
								if (_player != NULL)
								{
									float distance = NtlGetDistance(110.86, -100.63, _player->m_position.x, _player->m_position.z);
									if (distance <= 100.0f)
									{
										SendTSUpdateEventNfy(TS_TYPE_QUEST_CS, 16880);
										Timer.stop(idTimer);
									}
									else
									{
										sLog.outBasic("Out of range of goku (110.86, -100.63), player (%f, %f)", _player->m_position.x, _player->m_position.z);
									}
								}
							}
							catch (int e)
							{
								sLog.outError("Error to get player position");
								Timer.stop(idTimer);
							}

							}, 1000, idTimer, idTimer);
					}
					else if (sendSvrEvt->GetSvrEvtID() == 16885)
					{
						SendCharDirectPlay(true, 1, 1043);

						int idTimer = Timer.GetNewId();
						Timer.setTimeout([&]() {
							SendTSUpdateEventNfy(TS_TYPE_QUEST_CS, 16886);
							}, 31000, idTimer);
					}
					else if (sendSvrEvt->GetSvrEvtID() == 16890)
					{
						SendTSUpdateEventNfy(TS_TYPE_QUEST_CS, 16900);
					}
					// TLQ3 --------------------
					else
					{
						...
						if (result == GAME_SUCCESS && createdItem.tblidx != INVALID_TBLIDX)
						{
							...
							if (createdItem.tblidx == 99096)
							{
								...
								if (quest)
								{
									...
									quest->questSubCls.curQuestId = tid;
									quest->questSubCls.majinKarmaObject = true;
			...
			case DBO_ACT_TYPE_ID_ACT_WORLDPLAYSCRIPT:
			{
				...
				if (worldPlayScript->IsStart() && worldPlayScript->GetScriptID() == 6037)
				{
					...
					if (quest)
					{
						quest->questSubCls.npcHandle = SpawnNPCByTblidxQuestDende(3174103);
				...
				if (worldPlayScript->GetScriptID() == 11604)
				{
					...
					if (quest)
					{
						sLog.outBasic("1 New system: NPC HANDLE %d", quest->tlq1Info.handleNpc);
					}
					//sLog.outDebug("NPC HANDLE %d", _player->GetAttributesManager()->tlq1Info.handleNpc);
					ConvertGohanMobNPC(7451101, tid);
				}
				if (worldPlayScript->GetScriptID() == 11603 && tid == 11603)
				{
					//_player->GetAttributesManager()->tlq1Info.handleNpc = CreateNPCGohanTLQ1();
					//sLog.outDebug("NPC HANDLE %d", _player->GetAttributesManager()->tlq1Info.handleNpc);
					HOBJECT handleNpc = CreateNPCGohanTLQ1();
					_player->GetQuestManager()->AddNPCSpawnedQuest(handleNpc, tid);
					_player->GetQuestManager()->handleNpcTlq1Temp = handleNpc;
					QuestData* quest = _player->GetQuestManager()->FindQuestById(tid);
					if (quest)
					{
						quest->tlq1Info.handleNpc = handleNpc;
						sLog.outBasic("3 New system: Handle Gohan %d", handleNpc);
					}
				}
				// TLQ 1 -----------------------
				break;
			...
			case DBO_ACT_TYPE_ID_ACT_EXCEPT_TIMER_S:
			{
				...
				if (timerS)
				{
					...
					if (tid == 6062)
					{
						Timer.setTimeout([&](NTL_TS_T_ID questId) {
							QuestData* quest = _player->GetQuestManager()->FindQuestById(questId);
							if (quest)
							{
								sLog.outBasic("Time finish %d quest %d %d", quest->questSubCls.objData[0].evtId, questId, tid);
								SendTSUpdateEventNfy(TS_TYPE_QUEST_CS, quest->questSubCls.objData[0].evtId);
							}
			...
			{
				CDboTSActBroadMsg* broadMsg = ((CDboTSActBroadMsg*)contGAct->GetChildEntity(i));
				if (broadMsg)
				{
					sLog.outDebug("DisplayTime %d, excType %d ownerCond %d ownrTblidx %d ownerType %d questText %d uiBallon %d uidirType %d shideshowType %d uispeechtype %d", 
						broadMsg->GetDisplayTime(), broadMsg->GetExcutionType(), broadMsg->GetOwnerCondition(), broadMsg->GetOwnerTableIndex(),
						broadMsg->GetOwnerType(), broadMsg->GetQuestTextTableIndex(), broadMsg->GetUIBalloonShapeType(), broadMsg->GetUIDirType(),
						broadMsg->GetUIShowHideDirType(), broadMsg->GetUISpeechDirType());

					sGU_BROAD_MESSAGE_BEG_NFY nfy;
					nfy.wPacketSize = sizeof(sGU_BROAD_MESSAGE_BEG_NFY) - 2;
					nfy.wOpCode = GU_BROAD_MESSAGE_BEG_NFY;
					nfy.byUIDirType = broadMsg->GetUIDirType();
					nfy.byUIShowHideTypel = broadMsg->GetUIShowHideDirType();
					nfy.byUIBalloonShapeType = broadMsg->GetUIBalloonShapeType();
					nfy.byUISpeechDirType = broadMsg->GetUISpeechDirType();
					nfy.byOwnerCondition = broadMsg->GetOwnerCondition();
					nfy.byOwnerType = broadMsg->GetOwnerType();
					nfy.uiOwnerTblIdx = broadMsg->GetOwnerTableIndex();
					nfy.uiQuestTextTblidx = broadMsg->GetQuestTextTableIndex();
					nfy.fDisplayTime = broadMsg->GetDisplayTime();
					SendPacket((char*)&nfy, sizeof(sGU_BROAD_MESSAGE_BEG_NFY));
				}
				break;
			}
							...
							if (NpcInfo)
							{
								...
								Timer.setInterval([&](int idTimer) {
									...
									{ 
										if (_player != NULL)
										{
											float distance = NtlGetDistance(-123.44, -16.34, _player->m_position.x, _player->m_position.z);
											if (distance <= 50)
											{
									...
									{
										Timer.stop(idTimer);
										sLog.outError("Error to get player position to milk");
									}
...
ResultCodes WorldSession::ProcessTsContGCond(CDboTSContGCond * contGCond, NTL_TS_T_ID tid, NTL_TS_TC_ID tcId)
{
	for (int i = 0; i < contGCond->GetNumOfChildEntity(); i++)
	{
		...
		switch (contGCond->GetChildEntity(i)->GetEntityType())
		{
			...
			case DBO_EVENT_TYPE_ID_RCV_SVR_EVT:
			{
				CDboTSRcvSvrEvt* rcvSvrEvt = (CDboTSRcvSvrEvt*)contGCond->GetChildEntity(i);
				if (rcvSvrEvt)
				{
					sLog.outDebug("Quest evt id %d", rcvSvrEvt->GetEvtID());
					// TLQ1 -----------------------------
					if (rcvSvrEvt->GetEvtID() == 16030)
					{
						HOBJECT handleNpc = CreateNPCGohanTLQ1();
						_player->GetQuestManager()->AddNPCSpawnedQuest(handleNpc, tid);
						_player->GetQuestManager()->handleNpcTlq1Temp = handleNpc;
						...
						{
							quest->tlq1Info.handleNpc = handleNpc;
							sLog.outBasic("2 New system: Handle Gohan %d", handleNpc);
						}
					}
					// TLQ1 -----------------------------
					// TLQ3 -----------------------------
					...
					else if (rcvSvrEvt->GetEvtID() == 16830)
					{
						//Aqui deberia caminar milk luego enviarse el event
						SendTSUpdateEventNfy(TS_TYPE_QUEST_CS, 16840);
					}
					else if (rcvSvrEvt->GetEvtID() == 16856)
					{
						HOBJECT handleNpc3 = CreateNPCNpc3TLQ3();

						_player->GetQuestManager()->AddNPCSpawnedQuest(handleNpc3, tid);

						QuestData* quest = _player->GetQuestManager()->FindQuestById(tid);
						if (quest)
						{
							quest->tlq3Info.handleNpc3 = handleNpc3;
							sLog.outDetail("NPC HANDLE %d ADD TO QUEST %d", handleNpc3, quest->QuestID);
						}
					}
					// TLQ3 -----------------------------
				}
...
ResultCodes WorldSession::FindQuestInformation(sUG_TS_CONFIRM_STEP_REQ * req)
{
	...
	switch (contBase->GetEntityType())
	{
		...
		case DBO_CONT_TYPE_ID_CONT_GCOND:
		{
			...
			if (ProcessTsContGCond(contGCond, req->tId, req->tcCurId) == RESULT_FAIL)
			{
				return RESULT_FAIL;
			}
...
HOBJECT WorldSession::ConvertGohanMobNPC(TBLIDX mobTblidx, NTL_TS_T_ID tid)
{
	HOBJECT handleNpc = _player->GetTarget();
	QuestData* quest = _player->GetQuestManager()->FindQuestById(tid);
	if (quest)
	{
		handleNpc = quest->tlq1Info.handleNpc;
...
HOBJECT WorldSession::ConvertOxSatanMobNPC(TBLIDX mobTblidx, NTL_TS_T_ID tid)
{
	...
	if (curr_Npc)
	{
		...
		if (pMOBTblData != NULL)
		{
			...
			spawnData.fLastWalkingSpeed = 0.736;
			spawnData.fLastRunningSpeed = 6.0;
			spawnData.fLastAirgSpeed = 6.0;
			spawnData.fLastAirgDashSpeed = 0.736;
			spawnData.fLastAirgDashAccelSpeed = 0.736;
			spawnData.AttackSpeedRate = 1000;
			spawnData.SkillAnnimationSpeedModifier = 100.0;
...
HOBJECT WorldSession::CreateNPCOxSatanTLQ2()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(2991101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.fLastWalkingSpeed = 0.72600001;
		spawnData.fLastRunningSpeed = 7.0;
		spawnData.fLastAirSpeed = 7.0;
		spawnData.fLastAirDashSpeed = 0.72600001;
		spawnData.fLastAirDashAccelSpeed = 0.72600001;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 349.418;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = -92.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = 129.10001;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 1.;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.0;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCMilkTLQ2()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(1711201));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.fLastRunningSpeed = 7.0;
		spawnData.fLastAirSpeed = 7.0;
		spawnData.fLastAirDashSpeed = 2.;
		spawnData.fLastAirDashAccelSpeed = 2.;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 137.2;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = -88.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -199.17999;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.52999997;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.85000002;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCMilk2TLQ2()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(1711101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.State.sCharStateBase.vCurLoc.y = -88.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -199.17999;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 0.5;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.87;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCGokuTLQ2()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(2741101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.fLastWalkingSpeed = 2.0;
		spawnData.fLastRunningSpeed = 7.0;
		spawnData.fLastAirSpeed = 7.0;
		spawnData.fLastAirDashSpeed = 2.0;
		spawnData.fLastAirDashAccelSpeed = 2.0;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 350.85001;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = -92.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = 131.06;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 0.92000002;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = -0.38999999;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCGoku2TLQ2()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(2741101));
	if (pNPCTblData != NULL)
	{
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 137.98;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = -88.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -198.06;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.28;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = -0.95999998;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateMobDino1TLQ2()
{
	MobTable* MobTable = sTBM.GetMobTable();
	sMOB_TBLDAT* pMOBTblData = reinterpret_cast<sMOB_TBLDAT*>(MobTable->FindData(8211107));
	if (pMOBTblData != NULL)
	{
		SpawnMOB spawnData;
		memset(&spawnData, 0, sizeof(SpawnMOB));
		...
		spawnData.maxLP = pMOBTblData->wBasic_LP;
		spawnData.Size = 10;
		spawnData.Type = OBJTYPE_NPC;
		spawnData.Tblidx = pMOBTblData->tblidx;

		spawnData.fLastWalkingSpeed = 2.76;
		spawnData.fLastRunningSpeed = 6.8000002;
		spawnData.fLastAirgSpeed = 6.8000002;
		spawnData.fLastAirgDashSpeed = 2.76;
		spawnData.fLastAirgDashAccelSpeed = 2.76;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnnimationSpeedModifier = 100;

		sLog.outDebug("WalkSpeed %f run %d runOrigin %f walkOrigin %f",
			pMOBTblData->fWalk_Speed, pMOBTblData->fRun_Speed, pMOBTblData->fRun_Speed_Origin, pMOBTblData->fWalk_Speed_Origin);
		spawnData.TBLIDXMovementAcionPattern = 5;
		spawnData.Level = pMOBTblData->byLevel;

		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 138.98;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = -88.199997;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -200.57001;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.54075766;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = -0.84117848;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
		...
		if (pMOBTblData)
		{
			...
			{
				created_Mob->GetMapRef().link(this->_player->GetMap(), created_Mob);
				sLog.outString("Mob ID %d inserted into map", pMOBTblData->tblidx);
				return handle;
				//_player->GetAttributesManager()->lastNPCQuest = INVALID_TBLIDX;
				/*Timer.setTimeout([&]() {
					created_Npc->GetState()->sCharStateDetail.sCharStateDestMove.byMoveFlag = 1;
...
HOBJECT	WorldSession::CreateMobDino2TLQ2()
{
	MobTable* MobTable = sTBM.GetMobTable();
	sMOB_TBLDAT* pMOBTblData = reinterpret_cast<sMOB_TBLDAT*>(MobTable->FindData(8211108));
	if (pMOBTblData != NULL)
	{
		SpawnMOB spawnData;
		memset(&spawnData, 0, sizeof(SpawnMOB));
		...
		spawnData.fLastWalkingSpeed = 3.5880001;
		spawnData.fLastRunningSpeed = 6.8000002;
		spawnData.fLastAirgSpeed = 6.8000002;
		spawnData.fLastAirgDashSpeed = 3.5880001;
		spawnData.fLastAirgDashAccelSpeed = 3.5880001;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 188.22;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = -91.690002;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -269.42999;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.47110841;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.88207531;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateMobDino3TLQ2()
{
	MobTable* MobTable = sTBM.GetMobTable();
	sMOB_TBLDAT* pMOBTblData = reinterpret_cast<sMOB_TBLDAT*>(MobTable->FindData(8211109));
	if (pMOBTblData != NULL)
	{
		SpawnMOB spawnData;
		memset(&spawnData, 0, sizeof(SpawnMOB));
...
HOBJECT	WorldSession::CreateMobDino4TLQ2()
{
	MobTable* MobTable = sTBM.GetMobTable();
	sMOB_TBLDAT* pMOBTblData = reinterpret_cast<sMOB_TBLDAT*>(MobTable->FindData(8211110));
	if (pMOBTblData != NULL)
	{
		SpawnMOB spawnData;
		memset(&spawnData, 0, sizeof(SpawnMOB));
		...
		spawnData.fLastWalkingSpeed = 4.1399999;
		spawnData.fLastRunningSpeed = 6.8000002;
		spawnData.fLastAirgSpeed = 6.8000002;
		spawnData.fLastAirgDashSpeed = 4.1399999;
		spawnData.fLastAirgDashAccelSpeed = 4.1399999;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 188.25;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = -91.690002;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -269.53;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.47110841;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.88207531;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCOolongTLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(8671101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.fLastWalkingSpeed = 2.5999999;
		spawnData.fLastRunningSpeed = 8.;
		spawnData.fLastAirSpeed = 8.;
		spawnData.fLastAirDashSpeed = 2.5999999;
		spawnData.fLastAirDashAccelSpeed = 2.5999999;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.vCurLoc.x = -123.44;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -13.67;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 0.0;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = -1.;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCGokuTLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(2761801));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.fLastWalkingSpeed = 1.4;
		spawnData.fLastRunningSpeed = 7.;
		spawnData.fLastAirSpeed = 7.;
		spawnData.fLastAirDashSpeed = 1.4;
		spawnData.fLastAirDashAccelSpeed = 1.4;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 40.049999;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -40.150002;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 0.87;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = -0.47999999;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCNpc1TLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(3251101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = -76.190002;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -12.47;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 0.0;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 1.;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCNpc2TLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(3252101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = -76.220001;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -13.82;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 0.0;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 1.;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCNpc3TLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(1331101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = -79.449997;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = 3.7;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.62;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.79000002;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCPicoloTLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(5551105));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 89.360001;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.200001;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -100.93;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 0.94999999;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.33000001;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCTenshinhanTLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(7042101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 111.61;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.200001;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -102.32;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.95999998;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.28;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCGoku2TLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(2761801));
	if (pNPCTblData != NULL)
	{
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 110.86;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.200001;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -100.63;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.99000001;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = -0.1;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCKrillinTLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(3511104));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.fLastWalkingSpeed = 1.;
		spawnData.fLastRunningSpeed = 8.;
		spawnData.fLastAirSpeed = 8.;
		spawnData.fLastAirDashSpeed = 1.;
		spawnData.fLastAirDashAccelSpeed = 1.;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 112.32;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.200001;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -101.15;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.91000003;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.41999999;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCChaozTLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(1582101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 112.55;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.200001;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -100.5;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -1.;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.41999999;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCYamchaTLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(9042101));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 113.03;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.200001;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -101.72;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -1.;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.41999999;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCMilk2TLQ3()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(1701101));
	if (pNPCTblData != NULL)
	{
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = 109.638;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 22.124;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -84.;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.642;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.767;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCTurtleTutorial()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(7611104));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.fLastWalkingSpeed = 1.149;
		spawnData.fLastRunningSpeed = 5.;
		spawnData.fLastAirSpeed = 5.;
		spawnData.fLastAirDashSpeed = 1.149;
		spawnData.fLastAirDashAccelSpeed = 1.149;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = -77.779999;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 0.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -138.33;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = -0.34;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = -0.94;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCTrunksTutorial()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(7511101));
	if (pNPCTblData != NULL)
	{
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = -101.64;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 0.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -143.82001;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 0.5;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = 0.0;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::CreateNPCRobotTutorial()
{
	NPCTable* NpcTable = sTBM.GetNpcTable();
	sNPC_TBLDAT* pNPCTblData = reinterpret_cast<sNPC_TBLDAT*>(NpcTable->FindData(2413107));
	if (pNPCTblData != NULL)
	{
		SpawnNPC spawnData;
		memset(&spawnData, 0, sizeof(SpawnNPC));
		...
		spawnData.fLastWalkingSpeed = 1.1160001;
		spawnData.fLastRunningSpeed = 5.;
		spawnData.fLastAirSpeed = 5.;
		spawnData.fLastAirDashSpeed = 1.1160001;
		spawnData.fLastAirDashAccelSpeed = 1.1160001;
		spawnData.AttackSpeedRate = 1000;
		spawnData.SkillAnimationSpeedModifier = 100;
		...
		spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
		spawnData.State.sCharStateBase.vCurLoc.x = -111.25;// _player->m_position.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurLoc.y = 0.;//_player->m_position.y;
		spawnData.State.sCharStateBase.vCurLoc.z = -96.93;//_player->m_position.z + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.x = 0.23;//_player->m_rotation.x + rand() % 5;
		spawnData.State.sCharStateBase.vCurDir.y = 0.0;// _player->m_rotation.y;
		spawnData.State.sCharStateBase.vCurDir.z = -0.97000003;//_player->m_rotation.z + rand() % 5;
		spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::ConvertNPC1MobNPC(TBLIDX mobTblidx, NTL_TS_T_ID tid)
{
	...
	{*/
		...
		if (pMOBTblData != NULL)
		{
			...
			spawnData.fLastWalkingSpeed = 1.464;
			spawnData.fLastRunningSpeed = 6.;
			spawnData.fLastAirgSpeed = 6.;
			spawnData.fLastAirgDashSpeed = 1.464;
			spawnData.fLastAirgDashAccelSpeed = 1.464;
			spawnData.AttackSpeedRate = 1000;
			spawnData.SkillAnnimationSpeedModifier = 100.0;
			spawnData.TBLIDXMovementAcionPattern = 5;
			spawnData.Level = pMOBTblData->byLevel;;
			...
			spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
			spawnData.State.sCharStateBase.vCurLoc.x = -122.11;
			spawnData.State.sCharStateBase.vCurLoc.y = 22.120001;
			spawnData.State.sCharStateBase.vCurLoc.z = -12.71;
			spawnData.State.sCharStateBase.vCurDir.x = -0.85573208;
			spawnData.State.sCharStateBase.vCurDir.y = 0.0;
			spawnData.State.sCharStateBase.vCurDir.z = -0.51741934;
			spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT	WorldSession::ConvertNPC2MobNPC(TBLIDX mobTblidx, NTL_TS_T_ID tid)
{
	...
	{*/
		...
		if (pMOBTblData != NULL)
		{
			...
			spawnData.fLastWalkingSpeed = 0.96200001;
			spawnData.fLastRunningSpeed = 6.;
			spawnData.fLastAirgSpeed = 6.;
			spawnData.fLastAirgDashSpeed = 0.96200001;
			spawnData.fLastAirgDashAccelSpeed = 0.96200001;
			spawnData.AttackSpeedRate = 1000;
			spawnData.SkillAnnimationSpeedModifier = 100.0;
			spawnData.TBLIDXMovementAcionPattern = 1;
			spawnData.Level = pMOBTblData->byLevel;;
			...
			spawnData.State.sCharStateBase.aspectState.sAspectStateBase.byAspectStateId = 255;
			spawnData.State.sCharStateBase.vCurLoc.x = -121.68;
			spawnData.State.sCharStateBase.vCurLoc.y = 22.09;
			spawnData.State.sCharStateBase.vCurLoc.z = -13.84;
			spawnData.State.sCharStateBase.vCurDir.x = -0.97780246;
			spawnData.State.sCharStateBase.vCurDir.y = 0.0;
			spawnData.State.sCharStateBase.vCurDir.z = 0.20952909;
			spawnData.State.sCharStateBase.byStateID = eCHARSTATE::CHARSTATE_SPAWNING;
...
HOBJECT WorldSession::ConvertNPC1MobNPC2(TBLIDX mobTblidx, NTL_TS_T_ID tid)
{
	HOBJECT handleNpc = _player->GetTarget();
	QuestData* quest = _player->GetQuestManager()->FindQuestById(tid);
	if (quest)
	{
		handleNpc = quest->tlq3Info.handleNpc1;
		sLog.outDebug("NPC1 HANDLES 1: %d 2 %d", quest->tlq3Info.handleNpc1, _player->GetTarget());
	}
	Npc* curr_Npc = static_cast<Npc*>(_player->GetFromList(handleNpc));
	if (curr_Npc)
	{
...
HOBJECT WorldSession::ConvertNPC2MobNPC2(TBLIDX mobTblidx, NTL_TS_T_ID tid)
{
	HOBJECT handleNpc = _player->GetTarget();
	QuestData* quest = _player->GetQuestManager()->FindQuestById(tid);
	if (quest)
	{
		handleNpc = quest->tlq3Info.handleNpc2;
		sLog.outDebug("NPC2 HANDLES 1: %d 2 %d", quest->tlq3Info.handleNpc2, _player->GetTarget());
	}
	Npc* curr_Npc = static_cast<Npc*>(_player->GetFromList(handleNpc));
	if (curr_Npc)
	{
...
HOBJECT WorldSession::ConvertNPC3MobNPC(TBLIDX mobTblidx, NTL_TS_T_ID tid)
{
	HOBJECT handleNpc = _player->GetTarget();
	QuestData* quest = _player->GetQuestManager()->FindQuestById(tid);
	if (quest)
	{
		handleNpc = quest->tlq3Info.handleNpc3;
		sLog.outDebug("NPC3 HANDLES 1: %d 2 %d", quest->tlq3Info.handleNpc3, _player->GetTarget());
	}
	Npc* curr_Npc = static_cast<Npc*>(_player->GetFromList(handleNpc));
	if (curr_Npc)
	{
		...
		if (pMOBTblData != NULL)
		{
			...
			spawnData.fLastWalkingSpeed = 1.5;
			spawnData.fLastRunningSpeed = 9.;
			spawnData.fLastAirgSpeed = 9.;
			spawnData.fLastAirgDashSpeed = 1.5;
			spawnData.fLastAirgDashAccelSpeed = 1.5;
			spawnData.AttackSpeedRate = 1000;
			spawnData.SkillAnnimationSpeedModifier = 100.0;
			spawnData.TBLIDXMovementAcionPattern = 5;
			spawnData.Level = pMOBTblData->byLevel;;
	...
	if (pNPCTblData != NULL)
	{
		...
		if (spawnTbl)
		{
			...
			if (pNPCTblData)
			{
				...
				{
					created_Npc->GetMapRef().link(this->_player->GetMap(), created_Npc);
					sLog.outString("Npc ID %d inserted into map", NPCTblidx);
					//_player->GetAttributesManager()->lastNPCQuest = INVALID_TBLIDX;
				}
				else
					delete created_Npc;
				...
				for (int i = 0; i < contAct->GetNumOfChildEntity(); i++)
				{
					...
					switch (contAct->GetChildEntity(i)->GetEntityType())
					{
						...
						case DBO_ACT_TYPE_ID_ACT_QITEM:
						{
							...
							if (qItem)
							{
								if (questData->questSubCls.majinKarmaObject)//(questData->QuestID == _player->GetAttributesManager()->questSubCls.curQuestId)
								{
									for (int slot = 0; slot < qItem->eMAX_TS_QITEM_COUNT; slot++)
									{
										sLog.outDebug("Type %d Itemidx %d count %d probability %f", qItem->GetQItemType(),
						...
						case DBO_ACT_TYPE_ID_ACT_OBJSTATE:
						{
							...
							if (objState)
							{
								...
								if (questData->questSubCls.inQuest)//(_player->GetAttributesManager()->questSubCls.inQuest)
								{
		...
		for (auto reference = ref.begin(); reference != ref.end(); ++reference)
		{
			if (reference->getSource()->GetHandle() == req->hTarget)
			{
				...
				if (quest)
				{
					...
					if (quest->questSubCls.inQuest)//(_player->GetAttributesManager()->questSubCls.inQuest)
					{
						for (int i = 0; i < 2; i++)
						{
							//New system
							if (quest)
							{
								...
								/*sLog.outDebug("Item %d trigger %d object founded %d", 
									...
									objTblidx);*/
								if (/*_player->GetAttributesManager()->questSubCls.objData[i].objTblidx*/quest->questSubCls.objData[i].objTblidx == objTblidx)
								{
							
									sLog.outDebug("Item chose %d mobs %d", objTblidx, quest->questSubCls.objData[i].mobsTblidx.size());
									//_player->GetAttributesManager()->questSubCls.objData[i].mobsTblidx.size());
									...
									FindObjectTriggerInformation(triggerId, quest, req->hTarget, objTblidx);
								}
							}
						}
					}
