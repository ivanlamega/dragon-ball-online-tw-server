...
void CDboTSQAgency::LoadCompletedQInfo( const sCOMPLETE_QUEST_INFO& sInfo )
{
	...
	{
		CNtlTSLog::Log( "Can not save the completed quest info. [%s]", TS_CODE_TRACE() );
	}
}
...
void CDboTSQAgency::LoadQuestProgressInfo_V0( CNtlTSTrigger* pTrigger, const sPROGRESS_QUEST_INFO& stQInfo )
{
	if ( 0 == pTrigger )
	{
		CNtlTSLog::Log( "The parameter \"pTrigger\" Must not be NULL. Info[%d]. [%s]", stQInfo.tId, TS_CODE_TRACE() );
		return;
	}
	...
	{
		CNtlTSLog::Log( "Trigger info mismatch. Info[%d,%s]. [%s]", pTrigger->GetID(), pTCtrl ? pTCtrl->GetClassName() : "NULL", TS_CODE_TRACE() );
	}
}
