...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(NG_NPC_SPAWN_RES)
	sNPC_BRIEF	sNpcBrief;
	sCHARSTATE	sNpcState;
	HOBJECT		hNpcHandle;
	HOBJECT		hPlayerHandle;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(NG_MOB_SPAWN_RES)
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(NG_MOB_UNSPAWN_RES)
	HOBJECT	hPlayerHandle;
	HOBJECT hMobHandle;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(NG_NPC_UNSPAWN_RES)
	HOBJECT	hPlayerHandle;
	HOBJECT	hNpcHandle;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(NG_REFRESH_SERIAL)
	HOBJECT	hLastSerial;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(NG_ADMIN_MOB_SPAWN)
	sMOB_BRIEF	sMobBrief;
	sCHARSTATE	sMobState;
	HOBJECT		hMobHandle;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(NG_ADMIN_NPC_SPAWN)
	sNPC_BRIEF	sNpcBrief;
	sCHARSTATE	sNpcState;
	HOBJECT		hNpcHandle;
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(NG_SHOP_BUY)
	HOBJECT	hPlayer;
	HOBJECT hNpc;
	BYTE	byItemsCount;
	TBLIDX  sItemTblidx[NTL_MAX_BUY_SHOPPING_CART];
	BYTE	byOrderedStack[NTL_MAX_BUY_SHOPPING_CART];
	DWORD	dwTotalZenny;
END_PROTOCOL()
//------------------------------------------------------------------
