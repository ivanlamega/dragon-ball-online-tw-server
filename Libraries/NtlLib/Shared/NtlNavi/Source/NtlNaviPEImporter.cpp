...
void CNtlNaviPE_ODProp::RunMultiThread( void )
{
	...
	if ( NULL == m_pPropInfo )
	{
		CNtlNaviLog::GetInstance()->Log( "[IMPORT] Can not import world outdoor property data. [%d, %s]", m_uiFieldID, m_strPath );
...
void CNtlNaviPE_IDProp::RunMultiThread( void )
{
	...
	if ( NULL == m_pPropInfo )
	{
		CNtlNaviLog::GetInstance()->Log( "[IMPORT] Can not import world indoor property data. [%d, %s]", m_uiBlockID, m_strPath );
