...
class cInt128
{
    ...
    bool _isNegative;
    unsigned __int64 _high;
    unsigned __int64 _low;
...
public:
    ...
    explicit cInt128(__int64 value)
    {
        _high = 0;
        if(value < 0)
        {
    ...
    void
    initialiseAsProduct(const __int64& lhs, const __int64& rhs)
    {
        nLargeIntegers::multiplyTo128Bits(lhs, rhs, _isNegative, _high, _low);
    }

    bool
    divideTo32BitQuotient(__int64 divisor, long& quotientResult) const
    {
        bool hasRemainder;
    ...
    void operator*=(__int64 rhs)
    {
        ...
        unsigned __int64 addToHigh = __umulh(rhsU, _low);
        _high *= rhsU;
        _high += addToHigh;
        _low *= rhsU;
        //... check for overflow
