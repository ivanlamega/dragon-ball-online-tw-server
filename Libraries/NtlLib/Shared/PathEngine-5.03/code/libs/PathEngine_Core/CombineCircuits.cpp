...
//        JoinInlineCorners(result);
    ...
    for(i = 0; i != SizeL(baseObstacles); ++i)
    {
        ...
        cLocalisedConvexShape lcs(baseObstacleShapes[i], positionOnPartitioned, expansionShape, &faceBuffer.front(), faceFlagsBuffer);

        tMeshCircuits circuitsToAdd;
        //... optimise - build lcs and generate circuits at the same time
        {
            cCollisionShape combinedShape(baseObstacleShapes[i], expansionShape);
            std::list<tFace> containedExternalFaces;
            CircuitsForOverlappingPolygon(positionOnPartitioned, combinedShape, containedExternalFaces, circuitsToAdd);
        }
