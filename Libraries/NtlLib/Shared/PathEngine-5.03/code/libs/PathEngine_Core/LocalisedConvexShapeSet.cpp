...
{
    _expansions.clear();
    _expansionLookup.clearSubVectors();
    _setCompletedCalled = false;
}
...
{
    std::vector<long> entriesPerFace(_expansionLookup.size(), 0);
    ...
    for(i = 0; i != _expansionLookup.size(); ++i)
    {
        _expansionLookup.reserveSubVector(entriesPerFace[i]);
    }
