...
public:
    ...
    cMarker(const cCircuitElement* element, long faceIndex) :
      _element(element),
    ...
    tFace face() const
    {
        return _element->getFaceCrossed(_faceIndex);
    }
...
cMarker::start(bool& inclusive) const
{
    ...
    {
        tEdge e = _element->getEdgeCrossed(_faceIndex - 1);
...
cMarker::end(bool& inclusive) const
{
    ...
    tEdge e = _element->getEdgeCrossed(_faceIndex);
...
public:
    ...
    void copyAndAddPartOfExistingElement(const cCircuitElement& existingElement, long firstFace, long lastFace);
    void copyAndAddEndOfExistingElement(const cCircuitElement& existingElement, long firstFace);
...
cCircuitBuilder::copyAndAddPartOfExistingElement(
            const cCircuitElement& existingElement, long firstFace, long lastFace)
{
...
cCircuitBuilder::copyAndAddEndOfExistingElement(
            const cCircuitElement& existingElement, long firstFace)
{
    long lastFace = existingElement.facesCrossed() - 1;
...
cCircuitBuilder::buildAround(const cMarker& from, const cMarker& to)
{
    ...
    long currentFaceIndex = from.index();
    const cCircuitElement* targetElement = to.element();
    long targetFaceIndex = to.index();
...
class cBuilder
{
    ...
    long _closingFaceIndex;
    const cCircuitElement* _currentElement;
    long _currentFaceIndex;

    void buildAroundTo(const cCircuitElement& element, long faceIndex);
...
void
cBuilder::buildAroundTo(const cCircuitElement& element, long faceIndex)
{
    ...
    _elementReferencedFlags(elementReferencedFlags)
{
    ...
    tFace cornerFace = cornerElement.getFaceCrossed(0);
    cElementCollisionInfo info;
    cEdgesTraversal traversal;
...
cBuilder::buildOutEvent(const cCircuitElement& targetElement, bool& addedZeroLength)
{
    ...
    tFace targetFace = targetElement.getFaceCrossed(0);
    ...
    if(info.atCorner)
    {
        if(lineOut.side(info.element->refLine()) != SIDE_LEFT)
        {
            ...
            // goes directly into a DIRECTION_BACK corner
                ...
                cEdgesTraversal traversal(info.element->getFaceCrossed(0));
...
cRightSilhouetteQueue::add(const cCircuitElement* toAdd)
{
    ...
    while(i != _queue.end())
    {
        ...
        if(side == SIDE_CENTRE)
        {
            long existingLength, toAddLength;
...
// =================== AddTargetsInFace()
// adds right silhouettes only, and with no collision check

//void
//AddTargetsInFace(const cCircuitElement& corner, eSilhouetteZone zone,
//           const std::vector<cCornerInFaceEntry>& cornerLookup,
//           long f,
//           cRightSilhouetteQueue& queue
//           )
//{
//    tPoint cornerP = corner.cornerPoint();
//
//    size_t i;
...
//        eSilhouetteZone targetZone = ZoneFor(corner.getPrev()->refLine(), corner.refLine(), targetP);
//        if(targetZone != zone)
//        {
//            continue;
//        }
...
//        tFace cornerF = corner.getFaceCrossed(0);
//        exits = TestTraversalToXY(cornerF, testLine, endF);
//        if(exits)
//        {
//            continue;
//        }
//        if(endF != targetEl->getFaceCrossed(0))
//        {
//            continue;
//        }
//
...
SilhouetteIsBeforeMarkerEnd(const cMarker& marker, const tLine& lineToSilhouette)
{
    ...
    tEdge e = element->getEdgeCrossed(faceIndex);
...
SilhouetteIsAtOrAfterMarkerStart(const cMarker& marker, const tLine& lineToSilhouette)
{
    ...
    {
        tEdge e = element->getEdgeCrossed(faceIndex - 1);
            ...
            SilhouetteIsBeforeMarkerEnd(current, queue.refLineToNext()))
        {
            ...
            else
            {
              // a build in event occurs
                ...
                {
                    tFace f = added.getFaceCrossed(i);
                    if(!facesReferenced[f.index()])
                    {
                        facesReferenced.set(f.index());
                        AddTargetsInFace(cornerIP, axisBefore, axisAfter, zone,
...
{
    cInternalPosition cornerIP(corner.getFaceCrossed(0), corner.cornerPoint());
      ...
      )
{
    ...
    cBuilder builder(lookup, elementReferencedFlags, centre, axisBefore, axisAfter);
    ...
    build_Common(cornerLookup, centre, axisBefore, axisAfter, zone, facesReferenced, builder, queue, result);
}
...
// traverse out
...

    {
        ...
        {
            tFace f = traversal.getFaceCrossed(i);
            if(!facesReferenced[f.index()])
            {
                facesReferenced.set(f.index());
...
//                builder.buildInEvent(*queue.next());
...

                {
                    ...
                    {
                        tFace f = added->getFaceCrossed(j);
                        if(!facesReferenced[f.index()])
                        {
                            facesReferenced.set(f.index());
                            AddTargetsInFace_360(
            ...
            if(i.atConvexCorner())
            {
                ...
                if(zoneFromMarker == SZ_LEFTSILHOUETTE)
                {
                  // a build out event occurs
                    ...
                    {
                        tFace markerFace = markerElement->getFaceCrossed(0);
