...
{
    long i = toRemove->_splitterIndex;
...
void
cTranslationBSP::performSplit(tMesh& mesh, std::vector<tEdge>& potentialSplitters, long splitterToUse, long& nextSubSetIndex)
{
...
// traverse out from splitter in both directions until boundary of current subset is reached
    ...
    long subSet = splitter.face()->_subSet;
    assertD(subSet >= 0);
    ...
    long thisSubTreeIndex = SizeL(_splitters);
...
void
cTranslationBSP::performSplit(tMesh& mesh, std::vector<tEdge>& potentialSplitters, long& nextSubSetIndex)
{
