...
static void
saveCollisionShape(const cCollisionShape& shape, iXMLOutputStream& os)
{
    os.openElement("shape");
    os.addAttribute("vertices", shape.size());
    long i;
    for(i = 0; i < shape.size(); ++i)
    {
        os.openElement("vertex");
        os.addAttribute("x", shape.getVertex(i).getX());
        os.addAttribute("y", shape.getVertex(i).getY());
        os.closeElement("vertex");
    }
    os.closeElement("shape");
}
static bool
savedCollisionShapeMatches(const cCollisionShape& shape, iXMLInputStream& is)
{
    const char *const* attributes = is.openElement("shape");
    long vertices = ValueAsLong(attributes, "vertices");
    ...
    for(i = 0; i < shape.size(); ++i)
    {
        ...
        is.closeElement();
        if(ValueAsLong(attributes, "x") != shape.getVertex(i).getX()
         || ValueAsLong(attributes, "y") != shape.getVertex(i).getY())
        {
            is.closeElement();
            return false;
        }
...
void
cMesh::addElementCountRequirement(long elementCount)
{
...
//    GenerateCircuitsCheckSum(*_baseCircuits, generator);
    ...
    if(!_baseSurfaceTypeCosts.empty())
    {
        ...
        generator << static_cast<long>(_surfaceTypes->size());
...
void
cMesh::releasePreprocessFor(long i)
{
    assertD(i != -1);
...
SaveObstacles(
        ...
        const std::vector<cCollisionShape>& collisionShapes,
        ...
        )
{
    ...
    for(i = 0; i < SizeL(infos); ++i)
    {
        ...
        }
        {
            {
                std::ostringstream oss;
//                const cCollisionShape& cs = *info._shape;
                const cCollisionShape& cs = collisionShapes[i];
                oss << cs.getVertex(0).getX() << "," << cs.getVertex(0).getY();
                long j;
                for(j = 1; j < cs.size(); j++)
                {
                    oss << "," << cs.getVertex(j).getX() << "," << cs.getVertex(j).getY();
                }
...
//    }
    ...
    if(!_obstructionIDs.empty())
    {
        os.openElement("namedObstacles");
        SaveObstacles(_obstructions, _obstructionShapes, &_obstructionIDs, os);
...
{
    long i = shape->getPreprocessIndex();
...
static void
DoSaveCollisionPreprocess(cCollisionPreprocess& cp, long meshCheckSum, iShape* shape, iXMLOutputStream& os)
{
    cShape* cs = static_cast<cShape*>(shape);
    long i = cs->getPreprocessIndex();
...
cMesh::constructorCommon(const char *const* options)
{
    ...
    if(_sectionIDs)
    {
        ...
        {
            if((*_sectionIDs)[i] > topIndex)
            {
                topIndex = (*_sectionIDs)[i];
            }
        }
        _terrainCallBacks.resize(topIndex + 1, 0);
        _sectionBounds.resize((topIndex + 1) * 2, tPoint(0,0,0));
        std::vector<char> terrainBoundInitialised(topIndex + 1, false);
        for(i = 0; i < SizeL(*_sectionIDs); ++i)
        {
            long terrainLayer = (*_sectionIDs)[i];
            if(terrainLayer == -1)
            {
                continue;
            }
            ...
            if(!terrainBoundInitialised[terrainLayer])
            {
                _sectionBounds[terrainLayer * 2 + 0] = bottomLeft;
                _sectionBounds[terrainLayer * 2 + 1] = topRight;
                terrainBoundInitialised[terrainLayer] = true;
            }
            else
            {
                _sectionBounds[terrainLayer * 2 + 0].updateAsMinimumBound(bottomLeft);
                _sectionBounds[terrainLayer * 2 + 1].updateAsMaximumBound(topRight);
            }
        }
    }
...
cMesh::shrinkStuffToFit()
{
    _mesh3D->shrinkToFit();
    _mesh2D->shrinkToFit();
//    ShrinkToFit(_baseObstacleHeldShapes);
    ...
    ShrinkToFit(_obstructionIDs);
//    ShrinkToFit(_obstructionHeldShapes);
    ShrinkToFit(_obstructionShapes);
    ShrinkToFit(_obstructions);
          ...
          )
{
    ...
    PROFILER_CHANGES("cMesh::cMesh (with buffer) burn in base obstacles");
    {
        ...
        const std::vector<cCollisionShape>& collisionShapes = baseObstaclesLoader._shapes;
...
//            iShape* shape = nSingletons::pathEngine().newShape_Internal(*infos[i]._shape);
    ...
    PROFILER_CHANGES("cMesh::cMesh (with buffer) store anchors and shapes");
    {
        ...
        const std::vector<cCollisionShape>& collisionShapes = namedObstaclesLoader._shapes;
...
//    {
//        MEMORY_REPORT_SCOPE("cMesh - base obstacle held shapes");
//        CompletelyClear(_baseObstacleHeldShapes);
//    }
    {
    ...
    }
    {
        ...
        CompletelyClear(_obstructionIDs);
//        CompletelyClear(_obstructionHeldShapes);
        CompletelyClear(_obstructionShapes);
        CompletelyClear(_obstructions);
    }
    {
...
void
cMesh::get3DFaceVertex(long faceIndex, long vertexIndexInFace, long& x, long& y, float& z) const
{
    cMutexLock lock(_meshMutex);
...
long
cMesh::get3DFaceConnection(long faceIndex, long edgeIndexInFace) const
{
    cMutexLock lock(_meshMutex);
    INTERFACE_ASSERT(faceIndex >= 0 && faceIndex < getNumberOf3DFaces());
...
void
cMesh::get3DFaceNormal(long faceIndex, float& resultX, float& resultY, float& resultZ) const
{
    cMutexLock lock(_meshMutex);
    tFace_3D f = _mesh3D->faceAt(faceIndex);
...
long
cMesh::get3DFaceAttribute(long faceIndex, long attributeIndex) const
{
    cMutexLock lock(_meshMutex);
...
cMesh::getSurfaceType(const cPosition& position) const
{
    ...
    long f3DIndex = get3DFaceAtPosition(position);
...
void
cMesh::storeAnchor(const char* id, const cPosition& position, long orientation)
{
    cMutexLock lock(_meshMutex);
...
cPosition
cMesh::retrieveAnchor(const char* id, long& orientation) const
{
    cMutexLock lock(_meshMutex);
    INTERFACE_ASSERT(id);
    size_t i;
...
{
    cMutexLock lock(_meshMutex);
    return static_cast<long>(_anchorIDs.size());
}
void
cMesh::retrieveAnchorByIndex(long index, cPosition& position, long& orientation, const char*& id) const
{
    cMutexLock lock(_meshMutex);
...
cMesh::storeNamedObstacle(const char* id, iAgent* agent)
{
    ...
    INTERFACE_ASSERT(id);
    size_t i;
    for(i = 0; i < _obstructionIDs.size(); i++)
    {
        if(_obstructionIDs[i] == id)
        {
            break;
        }
    }
    if(i != _obstructionIDs.size())
    {
...
//    _obstructionHeldShapes.push_back(agentC->getShapeHandle());

    long x, y;
    agent->getTraverseCostDirection(x, y);

    cObstacleInfo info(agentC->getInternalPosition(),
//                       &agentC->refShape().refCS(),
                       agent->getTraverseCost(),
                       tPoint(x, y, DIRECTION_VECTOR_RANGE)
                       );
    _obstructions.push_back(info);

    _obstructionShapes.push_back(agentC->refShape().refCS());

    _obstructionIDs.push_back(id);
}
iAgent*
cMesh::retrieveNamedObstacle(const char* id) const
{
    ...
    for(i = 0; i < _obstructionIDs.size(); i++)
    {
        if(_obstructionIDs[i] == id)
        {
            ...
            cPosition p;
            convertPosition(_obstructions[i]._position, p);
//            iAgent* result = placeAgent(_obstructionHeldShapes[i].get(), p);
            iAgent* result = placeAgent(_obstructionShapes[i], p);
            result->setTraverseCost(_obstructions[i]._costToTraverse);
            tPoint v = _obstructions[i]._directionForCostToTraverse;
...
{
    cMutexLock lock(_meshMutex);
    return static_cast<long>(_obstructionIDs.size());
}
void
cMesh::retrieveNamedObstacleByIndex(long index, iAgent*& agent, const char*& id) const
{
    cMutexLock lock(_meshMutex);
    INTERFACE_ASSERT(index >= 0 && index < getNumberOfNamedObstacles());
    id = _obstructionIDs[index].c_str();

    cPosition p;
    convertPosition(_obstructions[index]._position, p);
//    agent = placeAgent(_obstructionHeldShapes[index].get(), p);
    agent = placeAgent(_obstructionShapes[index], p);
    agent->setTraverseCost(_obstructions[index]._costToTraverse);
    tPoint v = _obstructions[index]._directionForCostToTraverse;
    agent->setTraverseCostDirection(v.getX(), v.getY());
}

cPosition
cMesh::positionFor3DPoint(const long* point) const
{
...
{
    long lpoint[3];
    lpoint[0]=static_cast<long>(point[0]);
    lpoint[1]=static_cast<long>(point[1]);
    lpoint[2]=static_cast<long>(point[2]);
    return positionFor3DPoint(lpoint);
}

cPosition
cMesh::positionNear3DPoint(const long* point, long horizontalRange, long verticalRange) const
{
    return positionInSectionNear3DPoint(-1, point, horizontalRange, verticalRange);
}

cPosition
cMesh::positionInSection(long sectionID, long x, long y) const
{
    cMutexLock lock(_meshMutex);
    ...
    {
        long segments = _partitioning->queryFacesPotentiallyStabbed(p, &_partitioningResultsBuffer.front());
        long i;
        for(i = 0; i < segments; i++)
        {
            long f3DIndex = _partitioningResultsBuffer[i];
...
cPosition
cMesh::positionInSectionFor3DPoint(long sectionID, const long* point) const
{
    cMutexLock lock(_meshMutex);
    if(sectionID != -1 && !_sectionIDs)
    {
...
cPosition
cMesh::positionInSectionFor3DPointF(long sectionID, const float* point) const
{
    long lpoint[3];
    lpoint[0]=static_cast<long>(point[0]);
    lpoint[1]=static_cast<long>(point[1]);
...
cPosition
cMesh::positionInSectionNear3DPoint(long sectionID, const long* point, long horizontalRange, long verticalRange) const
{
    ...
    {
        long i;
        long faces = _partitioning->queryFacesPotentiallyOverlapped(
        ...
        for(i = 0; i < faces; i++)
        {
            ...
            long groundHeight = static_cast<long>(heightAtPointIn3DFace(f3DIndex, candidateP));
...
cPosition
cMesh::generateRandomPositionLocally(const cPosition& centre, long range) const
{
    cMutexLock lock(_meshMutex);
    ...
    cLocalisedConvexShape bounds(qc, centreIP, range);
    ...
    exitQuery(qc);
    const std::vector<long>& facesOverlapped = bounds.refFacesOverlapped();
    assertD(!facesOverlapped.empty());
    while(1)
    {
        long r = nSingletons::pathEngine().random31();
        long rX = nSingletons::pathEngine().random31();
        long rY = nSingletons::pathEngine().random31();

        long faceIndex = facesOverlapped[r % facesOverlapped.size()];
        ...
        {
            if(!bounds.pointCollides2D_Inverted(p))
            {
                cPosition result;
                result.x = p.getX();
                result.y = p.getY();
...
cPosition
cMesh::generateRandomPositionInSection(long sectionID) const
{
    cMutexLock lock(_meshMutex);
  	INTERFACE_ASSERT(sectionID >= 0);
    if(!_sectionIDs || sectionID < 0 || sectionID >= SizeL(_sectionBounds) / 2)
    {
        return cPosition();
    }
    tPoint bottomLeft = _sectionBounds[sectionID * 2 + 0];
    tPoint topRight = _sectionBounds[sectionID * 2 + 1];
    tPoint range = topRight - bottomLeft;
    cPosition result;
    long i;
    for(i = 0; i < 10; i++)
...
long cMesh::heightAtPosition(const cPosition& position) const
{
    cMutexLock lock(_meshMutex);
    return static_cast<long>(heightAtPositionF(position));
}
...
cMesh::heightAtPositionF(const cPosition& position) const
{
    ...
    if(_sectionIDs)
    {
        long terrainLayer = (*_sectionIDs)[fIndex];
        if(terrainLayer != -1 && _terrainCallBacks[terrainLayer])
        {
...
long
cMesh::getCellForEndOfLine(const cPosition& start, long x, long y) const
{
    cMutexLock lock(_meshMutex);
...
cMesh::burnContextIntoMesh(iCollisionContext* contextI)
{
    ...
    {
        long sizeBefore = SizeL(_baseObstacleShapes);
        assertD(sizeBefore == SizeL(_baseObstacles));
        _baseObstacleShapes.reserve(sizeBefore + context->getNumberOfAgents());
        _baseObstacles.reserve(sizeBefore + context->getNumberOfAgents());
    }
...
//        _baseObstacleHeldShapes.push_back(agent->getShapeHandle());
...
//                           &agent->refShape().refCS(),    
        ...
        if(_allowCollisionWithoutPreprocess && agent->getCostToTraverse() == -1.f)
        {
            ...
            lcs = new cLocalisedConvexShape(qc, agent->refShape().refCS(), agent->getInternalPosition());
...
void
cMesh::setTerrainCallBack(long terrainLayer, iTerrainCallBack* callBack)
{
    cMutexLock lock(_meshMutex);
    INTERFACE_ASSERT(!_preprocessGenerated);
	INTERFACE_CHECK(terrainLayer >= 0 && terrainLayer < SizeL(_terrainCallBacks), "iMesh::setTerrainCallBack() called with invalid terrain layer");
...
long
cMesh::addOffMeshConnection(long fromEndPoint, long toEndPoint, long penalty)
{
...
cPosition
cMesh::getEndPoint(long index) const
{
    cMutexLock lock(_meshMutex);
...
void
cMesh::getOffMeshConnectionInfo(long index, long& fromEndPoint, long& toEndPoint, long& penalty) const
{
    cMutexLock lock(_meshMutex);
...
void
cMesh::loadCollisionPreprocessFor(iShape* shape, const char* dataBuffer, unsigned long bufferSize)
{
    cMutexLock lock(_meshMutex);
    INTERFACE_ASSERT(!_preprocessGenerationCompleted);
    INTERFACE_ASSERT(shapeCanCollide(shape) == false);
    ...
    long majorVersion, minorVersion, meshCheckSum;
...
void
cMesh::loadPathfindPreprocessFor(iShape* shape, const char* dataBuffer, unsigned long bufferSize)
{
    cMutexLock lock(_meshMutex);
    INTERFACE_ASSERT(!_preprocessGenerationCompleted);
    INTERFACE_ASSERT(shapeCanCollide(shape) == true);
    ...
    long majorVersion = ValueAsLong(attributes, "majorVersion");
    long minorVersion = ValueAsLong(attributes, "minorVersion");

    {
        long meshCheckSum = ValueAsLong(attributes, "meshCheckSum");
        makeCheckSumValid();
        if(meshCheckSum != _checkSum)
        {
...
cMesh::testLineCollision_XY(
                iShape* shape, 
                iCollisionContext* context, 
                const cPosition& start, long x, long y, long& cell) const
{
                ...
                long maximumdistance) const
{
    ...
    INTERFACE_ASSERT(maximumdistance>=0 && maximumdistance<=SHAPE_RANGE);
        ...
        ) const
{
    ...
    std::vector<long>* connectionIndices = 0;
        ...
        ) const
{
    ...
    INTERFACE_ASSERT(distanceAway>=0 && distanceAway<=SHAPE_RANGE);
    cShape* cs = static_cast<cShape*>(shape);
    long index = cs->getPreprocessIndex();
...
//        cHandle<cContextCollisionPreprocess> ccp = cc->getCollisionPreprocessFor(qc, index);
        cCompoundContextCollisionPreprocess cccp;
//        cc->addCollisionPreprocessFor(qc, index, cccp);
//        cpp = cc->getPathfindPreprocessFor(qc, index);
    ...
    // shorten the path to before the last collision
        ...
        tFace sectionStartF;
        long beforeLast = SizeL(pathPoints) - 2;
    ...
    std::vector<long>* pushedIndices;
    if(_pathfindPreprocess.get(index)->smallConvexWereSplit())
    {
        pushedIndices = 0;
        if(connectionIndices)
        {
            pushedIndices = new std::vector<long>;
        }
        if(context)
        {
...
iPath*
cMesh::constructPath(const cPosition* positionsBuffer, const long* connectionIndicesBuffer, long pathLength) const
{
...
iPath*
cMesh::constructPath_Reversed(const cPosition* positionsBuffer, const long* connectionIndicesBuffer, long pathLength) const
{
    INTERFACE_ASSERT(pathLength > 0);
    cPath* result = nSingletons::pathEngine().refPathPool().get();
    _heldPaths.push_front(result);
...
iPath*
cMesh::loadPath(const char* dataBuffer, unsigned long bufferSize) const
{
    if(bufferSize == 0)
    {
        return 0;
...
void
cMesh::renderLineOnGround(const cPosition& start, long endX, long endY, long originX, long originY, iRender3DLinesCallBack& callBack) const
{
...
cPosition
cMesh::positionInSectionInShape(long sectionID, long shapeOriginX, long shapeOriginY, long numberOfPoints, const long* coordinateData) const
{
    cMutexLock lock(_meshMutex);
    ...
    cCollisionShape cs(numberOfPoints, coordinateData);
    tPoint bottomLeft, topRight;
...
//    }
//
//    long shapeIndex = shape.getPreprocessIndex();
//
...
//        return;
//    }
//    long unwindTo = path.getIndexForEndUnwind(newEndPosition.p);
//
//#ifdef INTERFACE_CHECKING
//    {
//        long i;
...
//        return;
//    }
//    long unwindTo = path.getIndexForStartUnwind(newStartPosition.p);
//
//#ifdef INTERFACE_CHECKING
//    {
//        long i;
...
void
cMesh::notifyContextDestroyed(long index)
{
    cMutexLock lock(_heldContextsMutex);
    _heldContexts.release(index);
}
void
cMesh::notifyAgentDestroyed(long index)
{
    cMutexLock lock(_heldAgentsMutex);
    _heldAgents.release(index);
}
void
cMesh::notifyObstacleSetDestroyed(long index)
{
    cMutexLock lock(_heldObstacleSetsMutex);
...
bool
cMesh::shapeCanCollide(long index) const
{
    assertD(index != -1);
    return _collisionPreprocess.get(index).isSet();
}
bool
cMesh::shapeCanPathfind(long index) const
{
    assertD(index != -1);
    return _pathfindPreprocess.get(index).isSet();
}
...
tEdge cMesh::lookupEdge(long index) const
{
...
long cMesh::size() const
{
    return ref2D().faces();
}

cHandle<cCollisionPreprocess>
cMesh::getCollisionPreprocessFor(long index) const
{
    cMutexLock lock(_meshMutex);
...
cMesh::getPathfindPreprocessFor(long index) const
{
    cMutexLock lock(_meshMutex);
...
{
    long index = shape.getPreprocessIndex();
...
cMesh::refSilhouetteLookup(const cShape& shape) const
{
    assertD(shapeCanPathfind(&shape));
    long index = shape.getPreprocessIndex();
    return _pathfindPreprocess.get(index)->refSilhouetteLookup();
}
cPosition
cMesh::positionForBaseCorner(cQueryContext& qc, const cShape& shape, long stateIndex) const
{
    assertD(shapeCanPathfind(&shape));
    long index = shape.getPreprocessIndex();
    cInternalPosition ip;
    _pathfindPreprocess.get(index)->lookupPositionForBaseCorner(qc, stateIndex, ip);
    cPosition result;
    convertPosition(ip, result);
    return result;
}
...
bool
cMesh::testLineCollision_Internal(cQueryContext& qc, long shapeIndex, 
...
bool
cMesh::testLineCollision_Internal_ExceptSmallConvex(cQueryContext& qc, long shapeIndex, 
                                    cCollisionContext* context, 
                                    const cInternalPosition& start, const cInternalPosition& end) const
{
...
{
    long shapeIndex = FromI(shape)->getPreprocessIndex();
    cInternalPosition ip;
        ...
        ) const
{
    ...
    if(pp->smallConvexWereSplit())
    {
        ...
        std::vector<long>* beforeIndices = 0;
        if(context)
        {
            INTERFACE_ASSERT(&context->refMesh() == this);
        ...
        if(beforeIndices)
        {
            resultIndices = new std::vector<long>;
        }
        if(context)
        {
...
float
cMesh::heightAtPointIn3DFace(long f3DIndex, const tPoint& p) const
{
    if(_sectionIDs)
    {
        long terrainLayer = (*_sectionIDs)[f3DIndex];
        if(terrainLayer != -1 && _terrainCallBacks[terrainLayer])
        {
...
float
cMesh::heightAtPointIn3DFace(long f3DIndex, float x, float y) const
{
    if(_sectionIDs)
    {
        long terrainLayer = (*_sectionIDs)[f3DIndex];
        if(terrainLayer != -1 && _terrainCallBacks[terrainLayer])
        {
            long xRounded = static_cast<long>(x);
            long yRounded = static_cast<long>(y);
...
//        CircuitsForOverlappingPolygon(info._position, *info._shape, containedExternalFaces, *result);
        CircuitsForOverlappingPolygon(info._position, _baseObstacleShapes[i], containedExternalFaces, *result);
...
cMesh::refCollisionPreprocess(long index) const
{
    return *_collisionPreprocess.get(index);
}
const cPathfindPreprocess&
cMesh::refPathfindPreprocess(long index) const
{
    return *_pathfindPreprocess.get(index);
}
...
unsigned long
cMesh::lookupPenaltyForOffMeshConnection(long index) const
{
    assertD(_offMeshConnections);
...
cPosition
cMesh::translateFrom3D(long f3D, long x, long y) const
{
    tPoint p(x, y, WORLD_RANGE);
...
iAgent*
cMesh::placeAgent(const cCollisionShape& cs, const cPosition& position) const
{
