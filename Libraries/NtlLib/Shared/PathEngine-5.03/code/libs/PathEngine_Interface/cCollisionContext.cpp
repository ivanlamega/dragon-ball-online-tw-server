...
void
cCollisionContext::doSwapOutObstacleSet(long i) const
{
...
cCollisionContext::cCollisionContext(cMesh* mesh) :
 _obstacleSets(1),
 _pathfindPreprocess(nSingletons::pathEngine().numberOfPreprocessShapes(), 0)
{
    _wrapper = 0;
...
void
cCollisionContext::setIndexInMesh(long value)
{
...
iAgent* cCollisionContext::getAgent(long index) const
{
    cMutexLock lock(_mesh->refAgentsAndObstacleSetsMutex()); // to protect the _obstacleSets vector
...
iObstacleSet*
cCollisionContext::getObstacleSet(long index) const
{
    cMutexLock lock(_mesh->refAgentsAndObstacleSetsMutex()); // to protect the _obstacleSets vector
    reapObstacleSets();
...
void
cCollisionContext::setSurfaceTypeTraverseCost(long surfaceType, float cost)
{
...
void
cCollisionContext::setSurfaceTypeTraverseCostDirection(long surfaceType, long directionVectorX, long directionVectorY)
{
...
void
cCollisionContext::setQueryBounds(long startX, long startY, long endX, long endY)
{
    INTERFACE_CHECK(endX >= startX, "iCollisionContext::setQueryBounds() - endX must be >= startX.");
    INTERFACE_CHECK(endY >= startY, "iCollisionContext::setQueryBounds() - endY must be >= startY.");
    INTERFACE_ASSERT(startX >= -WORLD_RANGE && startY <= WORLD_RANGE);
    INTERFACE_ASSERT(endX >= -WORLD_RANGE && endY <= WORLD_RANGE);
    {
        cMutexLock lock(_internalMutex);
        _boundsMin = tPoint(startX, startY, WORLD_RANGE);
        _boundsMax = tPoint(endX, endY, WORLD_RANGE);
        _boundsValid = true;
    }
    invalidatePathfindingPreprocess();
}
...
void
cCollisionContext::addCollisionPreprocessFor(cQueryContext& qc, long index, cCompoundContextCollisionPreprocess& addTo) const
{
    cMutexLock lock(_mesh->refAgentsAndObstacleSetsMutex());
    long i;
    for(i = 0; i != SizeL(_obstacleSets); ++i)
            ...
            ) const
{
    ...
    tPoint boundsMin, boundsMax;
    {
        ...
        if(!_surfaceTypeCosts->empty())
        {
            ...
            _pathfindPreprocess[i]->addMesh3DSoftCorners(cornersToAdd);
        }
    }
    ...
    for(j = 0; j != _obstacleSets.size(); ++j)
    {
        ...
        _obstacleSets[j]->addAgentsToPathfindPreprocess(qc, i, *cp, *pp, boundsValid, boundsMin, boundsMax, *_pathfindPreprocess[i]);
    }

    _pathfindPreprocess[i]->setCompleted();
    dynamicPathfindPreprocess = _pathfindPreprocess[i];
}
