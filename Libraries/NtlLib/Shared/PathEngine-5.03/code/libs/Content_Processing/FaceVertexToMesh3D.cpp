...
static bool
FaceMarksPortal(const iFaceVertexMesh& mesh, long face)
{
...
static bool
FaceMarksTerrainPortal(const iFaceVertexMesh& mesh, long face)
{
    return mesh.faceAttribute(face, PE_FaceAttribute_MarksTerrainPortal) == 1;
}
static bool
FaceMarksExternalShape(const iFaceVertexMesh& mesh, long face)
{
    return mesh.faceAttribute(face, PE_FaceAttribute_MarksExternalShape) == 1;
}
static bool
FaceMarksConnectingEdge(const iFaceVertexMesh& mesh, long face)
{
                      ...
                      )
{
    ...
    for(i = 0; i < mesh.faces(); ++i)
    {
        ...
        long v1 = collapses.getCollapsedIndex(mesh.vertexIndex(i, 0));
        long v2 = collapses.getCollapsedIndex(mesh.vertexIndex(i, 1));
        if(v1 == v2)
        {
...
BuildMesh3D(const iFaceVertexMesh& collapsedFilter,
            ...
            tMesh& result,
            long part, std::vector<std::pair<long, long> >& faceMapping
            )
{
    long i;

    long sizeBefore = result.faces();
    ...
    for(i = 0; i < faceVertex.faces(); ++i)
    {
        ...
        faceMapping[result.faces()] = std::pair<long, long>(part, i);
        ...
        for(j = 0; j < 3; ++j)
        {
            long v = faceVertex.vertexIndex(i, j);
        ...
        )
{
    ...
    std::vector<long> lookupByEdge(faceVertex.faces() * 3, -1);
    long i;
    ...
    for(i = 0; i < SizeL(portals); ++i)
    {
        ...
        do
        {
            ...
            for(j = 0; j < 3; ++j)
            {
                long v = faceVertex.vertexIndex(f, j);
                vertexData[j]._x = faceVertex.vertexX(v);
                vertexData[j]._y = faceVertex.vertexY(v);
                edgeData[j]._z = faceVertex.vertexZ(v);
            }
      ...
      // check the shape has no holes
        ...
        do
        {
            long portalIndex = lookupByEdge[edgeFV.index()];
        ...
        )
{
    ...
    for(i = 0; i < mesh.faces(); ++i)
    {
        ...
        long v1 = mesh.getCollapsedIndex(i, 0);
        long v2 = mesh.getCollapsedIndex(i, 1);
        if(v1 == v2)
        {
...
static float
FaceIsAboveMaxSlope(const iFaceVertexMesh& mesh, long fIndex, float maxSlope)
{
...
{ 
    ...
    {
        if(filterBySlope
         && !FaceMarksPortal(faceVertex, i)
         && !FaceMarksConnectingEdge(faceVertex, i)
         && FaceIsAboveMaxSlope(faceVertex, i, maximumSlope)
         )
        {
