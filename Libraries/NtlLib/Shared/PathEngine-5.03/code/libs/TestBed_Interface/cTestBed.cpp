...
//    m_subscriber_node.erase();
//    _shapeSubscriber._subscriberNode.erase();
}
...
void cTestBed::setMesh(iMesh *mesh)
{
    ...
    _meshRenderInfo->build(_mesh->ref3D(),gip);
    GenerateMeshLOSPreprocess(_mesh->ref3D(), _losPreprocess);
    _pickedThisFrame=false;
}
void cTestBed::releaseMesh()
{
    ...
    assertD(_meshRenderInfo);
    _meshRenderInfo->clear();
//    delete _agentRenderPreprocess;
    _agentRenderPreprocess.deleteAll();

//    for(i = 0; i < _suppliedHeightFields.size(); ++i)
//    {
//        delete _suppliedHeightFields[i];
//    }
//    _suppliedHeightFields.clear();
...
//        const std::vector<long>* sectionIDs = _mesh->getSectionIDsVector();
        ...
        do
        {
            ...
            do
            {
//                if(sectionIDs && (*sectionIDs)[e.twin().face().index()] != -1)
//                {
//                     continue;
//                }
                _meshRenderInfo->drawEdge(e.twin().index());
            }
...
void cTestBed::draw3DMeshInternalEdges()
{
    if(_mesh)
    {
        gip->setZBias(-0.2f);
...
void cTestBed::drawSquareBounds(const cPosition &start, long distance)
{
    if(!_mesh)
        return;
    if(start.cell<0)
...
{
    if(_mesh)
        _meshRenderInfo->drawFace(f.index());
}
...
cTestBed::drawBurntInObstacles()
{
    ...
    if(!_baseCircuitsRecorder)
    {
        ...
        DrawMeshCircuits(*baseCircuits, _baseCircuitsRecorder, _mesh->refTranslation());
    }
    _baseCircuitsRecorder->play();
}
...
cTestBed::meshObstructsCamera()
{
    ...
    long faceIndex;
    return LineCollidesWithMesh(line, _losPreprocess, faceIndex);
}
void
