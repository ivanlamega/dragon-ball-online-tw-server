...
void DoFWrite(FILE *file, const char *buffer, unsigned long length)
{
        ...
        )
{
    ...
    cMeshesForSelectionSet meshes(theGroundExport().ip, prefix, useMaterialIDs, setTerrainLayer, processConnections, scale);
    ...
    if(addAnchors)
    {
        if(namedGround)
        {
            BuildAnchorsAndShapesFromMax_AllInScene(theGroundExport().ip, anchorsAndShapes, scale);
        }
        else
        {
            BuildAnchorsAndShapesFromMax(theGroundExport().ip, prefix, anchorsAndShapes, scale);
        }
    }
    ...
    if(mesh)
    {
        if(addAnchors)
        {
            AddAnchorsAndPinnedShapes(nSingletons::pathEngine(), mesh, anchorsAndShapes, "");
        }
...
DoExportNamedChunk(
        ...
        long maxScale, long translationRange,
        bool logErrors
        )
{
    if(logErrors)
    ...
    cMeshesForSelectionSet meshes(theGroundExport().ip, prefix, true, true, true, scale);
...
DoImport(const char *fileName, float scale, float anchorHeight)
{
    ...
    const char* format = "xml";
    long fileNameLength = static_cast<long>(strlen(fileName));
    if(fileNameLength >= 4
