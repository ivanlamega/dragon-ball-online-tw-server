                                      ...
                                      INode* node)
{
    ...
    if(strncmp(name, prefix, prefixLength) == 0)
    {
        ...
        else
        {
            ...
            _wrappers.push_back(new cMaxMeshWrapper(mesh, node->GetObjectTM(time), useMaterialIDs, setTerrainLayer, processConnections, scale));
        }
    }
    long i;
    for(i = 0; i < node->NumberOfChildren(); ++i)
    {
        add_Recursive(ip,
                    prefix, prefixLength,
                    useMaterialIDs, setTerrainLayer, processConnections, scale,
                    node->GetChildNode(i));
    }
}

cMeshesForSelectionSet::cMeshesForSelectionSet(Interface* ip, const char* prefix, bool useMaterialIDs, bool setTerrainLayer, bool processConnections, float scale)
{
    if(prefix)
    {
...
bool
cMeshesForSelectionSet::wasConverted(long index) const
{
