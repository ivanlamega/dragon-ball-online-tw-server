...
inline int
compareMagnitudes(const int N, const int N2, const unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
    int i=N2;
    while(--i>=N)
    {
...
inline bool
magnitudeGreaterThan(const int N, const int N2, const unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
...
inline void
addMagnitude(const int N, const int N2, unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
    unsigned long carry;
    int i;
...
inline void
subtractMagnitude(const int N, const int N2, unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
...
inline bool
sign(const int N, const unsigned long* lhsDigits)
{
    return lhsDigits[N] != 0;
}

inline void
constructFromLong(const int N, unsigned long* lhsDigits, long value)
{
    if(value>=0)
    {
        lhsDigits[0]=value;
    ...
    {
        assertD(static_cast<unsigned long>(value)!=0x80000000);
        lhsDigits[0]=-value;
        lhsDigits[N]=true;
    }
    int i;
...
inline void
constructFrom(const int N, const int N2, unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
...
inline void
clear(const int N, unsigned long* lhsDigits)
{
    int i;
    for(i = 0; i < N; i++)
        lhsDigits[i]=0;
...
inline bool
isNegative(const int N, const unsigned long* lhsDigits)
{
    return lhsDigits[N] != 0;
}
inline bool
isZero(const int N, const unsigned long* lhsDigits)
{
    int i;
    for(i = 0; i < N; i++)
    {
...
inline long
asLong(const int N, const unsigned long* lhsDigits)
{
    int i;
    for(i = 1; i < N; i++)
    {
        assertD(lhsDigits[i]==0); // too large to be represented in a long
    }
    assertD(lhsDigits[0]<=0x7fffffff); 
// note that this doesn't support -0x80000000
    if(lhsDigits[N])
        return -static_cast<long>(lhsDigits[0]);
    return static_cast<long>(lhsDigits[0]);
}
inline float
asFloat(const int N, const unsigned long* lhsDigits)
{
    int i=N-1;
    while(lhsDigits[i]==0)
    {
...
inline double
asDouble(const int N, const unsigned long* lhsDigits)
{
    int i=N-1;
    while(lhsDigits[i]==0)
    {
...
inline void
doNegate(const int N, unsigned long* lhsDigits)
{
...
inline void
negated(const int N, const unsigned long* lhsDigits, unsigned long* resultDigits)
{
    int i;
    for(i = 0; i < N; i++)
...
inline bool
equal(const int N, const int N2, const unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
...
inline bool
lessThan(const int N, const int N2, const unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
    bool result;
...
inline int
compare(const int N, const int N2, const unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
    int result;
    if(sign(N, lhsDigits)!=sign(N2, rhsDigits))
    {
...
inline void
multiplyByLong(const int N, unsigned long* lhsDigits, long rhs)
{
    if(rhs<0)
    {
        assertD(static_cast<unsigned long>(rhs)!=0x80000000);
        rhs=-rhs;
        lhsDigits[N]=!sign(N, lhsDigits);
    }
    unsigned long _rhs=static_cast<unsigned long>(rhs);
...
inline void
multiplyBy(const int N, const int N2, unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
//    unsigned long dest[N];
    unsigned long dest[MAXN];
    unsigned long overflow,carry;
    int i;
...
inline void
addTo(const int N, const int N2, unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
...
//            unsigned long resultDigits[N2 + 1];
            unsigned long resultDigits[MAXN + 1];
            long i;
            for(i = 0; i <= N2; i++)
            {
...
inline void
subtractFrom(const int N, const int N2, unsigned long* lhsDigits, const unsigned long* rhsDigits)
{
    if(rhsDigits[N2]==lhsDigits[N])
    {
    // subtract
...
//{
//    unsigned long h=lhsDigits[1];
//    unsigned long l=lhsDigits[0];
//    unsigned long h2=rhsDigits[1];
//    unsigned long l2=rhsDigits[0];
//
...
//inline void
//multiply(long a1, long a2, cLargeInteger2& result)
//{
//    unsigned long low;
//    unsigned long high;
...
inline void
multiply(const cLargeInteger2& source1, long source2, cLargeInteger4& dest)
{
    unsigned long carry;
    unsigned long high;
    ...
    unsigned long s2 = static_cast<unsigned long>(source2);
    ...
    {
        nAssemblyMath::add(carry, dest.digits[2], carry);
        // note that this cant carry because s2 is not a full unsigned long
        assertD(!carry); //.... remove this at some point because this is called very often
    }
}
...
inline bool
divide(const int N, const int N2, const unsigned long* dividendDigits,
                      const unsigned long* divisorDigits,
                      long& quotient_result)
{
...
//    //... but since quotient must fit in a long, perhaps this is 1
...
//    multipliedOut *= _q_top;
    ...
    unsigned long multipliedOutDigits[MAXN];
    constructFrom(N2 + 1, N2, multipliedOutDigits, divisorDigits);
    unsigned long qTopDigits[2];
    qTopDigits[0] = q_top;
    qTopDigits[1] = 0;
...
// cast to long and sort out sign

    assertD(q_top < 0x80000000); // overflow long representation - note negative max is not allowed
    quotient_result = static_cast<long>(q_top);
...
inline
cLargeInteger2::cLargeInteger2(long value)
{
    nLargeIntegers::constructFromLong(2, digits, value);
}
inline
cLargeInteger4::cLargeInteger4(long value)
{
...
inline void
operator+=(cLargeInteger2& lhs, long rhs)
{
    unsigned long rhsDigits[2];
...
inline cLargeInteger2
operator-(const cLargeInteger2& lhs, long rhs)
{
