        ...
        )
{
    ...
    for(long i = 0; i != fvMesh.vertices(); ++i)
    {
        MPoint p(fvMesh.vertexY(i), fvMesh.vertexZ(i), fvMesh.vertexX(i));
        vertices[i] = p / scale;
    }
    for(long i = 0; i != fvMesh.faces(); ++i)
    {
        for(long j = 0; j != 3; ++j)
        {
    ...
    {
        long sectionID = fvMesh.faceAttribute(i, PE_FaceAttribute_SectionID);
        ...
        else if(fvMesh.faceAttribute(i, PE_FaceAttribute_MarksPortal) == 1)
        {
