...
void CNtlTSGroup::AttachChildCont( CNtlTSCont* pChildCont )
{
	if ( 0 == pChildCont )
	{
		CNtlTSLog::Log( "Can not attach the container. ParamInfo[%s]. [%s]", "NULL", TS_CODE_TRACE() );
		return;
	}
...
CNtlTSCont* CNtlTSGroup::DetachChildCont( CNtlTSCont* pChildCont )
{
	if ( 0 == pChildCont )
	{
		CNtlTSLog::Log( "Can not detach the container. ParamInfo[%s]. [%s]", "NULL", TS_CODE_TRACE() );
		return 0;
	}
	...
	{
		CNtlTSLog::Log( "Can not find the container. ParamInfo[%d]. [%s]", pChildCont->GetID(), TS_CODE_TRACE() );
	}
...
void CNtlTSGroup::AttachChildForScript( CNtlTSScrObject* pChild )
{
	...
	{
		CNtlTSLog::Log( "Can not do type cast from CNtlTSScrObject to CNtlTSCont. ParamInfo[%s]. [%s]", pChild->GetClassName(), TS_CODE_TRACE() );
	}
}
