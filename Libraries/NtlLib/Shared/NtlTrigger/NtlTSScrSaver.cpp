...
bool CNtlTSScrSaver::Save( const std::string& strScriptName, std::vector<CNtlTSScrObject*>& vecRootObjList )
{
	...
	for ( ; itObj != vecRootObjList.end(); ++itObj )
	{
		...
		{
			CNtlTSLog::Log( "Saving is failed. Info[%s]. [%s]", strScriptName.c_str(), TS_CODE_TRACE() );
			return false;
		}
	}
	...
	{
		CNtlTSLog::Log( "Can not write to the file. Info[%s]. [%s]", strScriptName.c_str(), TS_CODE_TRACE() );
		return false;
	}
...
//	CNtlTSScrFactoryType::sPROPERTY_INFO* pPropInfo;
	...
	for ( ; itObj != defObjList.end(); ++itObj )
	{
		...
		{
			CNtlTSLog::Log( "Can not find the object type. Info[%s]. [%s]", stObjInfo.pObj->GetClassName(), TS_CODE_TRACE() );
			return false;
		}
...
bool CNtlTSScrSaver::BuildObjectInfoTree( CNtlTSScrObject* pRootObj, mapdef_ObjInfoList& defObjInfoList )
{
	...
	for ( ; itObj != defObjInfoList.end(); ++itObj )
	{
		...
		if ( 0 == stObjInfo.pObj->GetParentForScript() )
		{
			if ( pRootObjInfo )
			{
				CNtlTSLog::Log( "Only must have one parent. Info[%s]. [%s]", stObjInfo.pObj->GetClassName(), TS_CODE_TRACE() );
				return false;
			}
		...
		else
		{
			...
			{
				CNtlTSLog::Log( "Can not find the object. Info[%s]. [%s]", stObjInfo.pObj->GetClassName(), TS_CODE_TRACE() );
				return false;
			}
	...
	if ( 0 == pRootObjInfo )
	{
		CNtlTSLog::Log( "Can not find the root object. [%s]", TS_CODE_TRACE() );
		return false;
	}
...
bool CNtlTSScrSaver::WriteToFile( const std::string& strScriptName )
{
	...
	{
		CNtlTSLog::Log( "Can not open the file. [%s]. [%s]", strScriptName.c_str(), TS_CODE_TRACE() );
		return false;
	}
	...
	for ( ; it != m_defObjObjInfoMapList.end(); ++it )
	{
		...
		{
			fclose( pFile );
			CNtlTSLog::Log( "Writing the file is failed. [%s]. [%s]", strScriptName.c_str(), TS_CODE_TRACE() );
			return false;
		}
...
bool CNtlTSScrSaver::WriteObj( FILE* pFile, sOBJ_INFO* pstObjInfo, int nDepth )
{
	...
	for ( ; cit != citEnd; ++cit )
	{
		...
		case CNtlTSScrFactoryType::sPROPERTY_TYPE::eDATA_TYPE_STRING:
			...
			break;
		default:
			CNtlTSLog::Log( "Can not find the matching property type. Info[%s]. [%s]", cit->first.c_str(), TS_CODE_TRACE() );
			return false;
		}
	}
