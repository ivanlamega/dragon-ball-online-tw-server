...
public:
    ...
    {
        long startIndex = IndexForElement(el);
        long endIndex = _tree.transformDown(startIndex, p);
    ...
    {
        long startIndex = IndexForElement(el);
        long endIndex = _tree.transformUp(startIndex, p);
...
// methods passed on to the mesh
    ...
    long faces() const {return _transformedMesh->faces();}
    long vertices() const {return _transformedMesh->vertices();}
    long edges() const {return _transformedMesh->edges();}
    ...
    face faceAt(long index) {return _transformedMesh->faceAt(index);}
    edge edgeAt(long index) {return _transformedMesh->edgeAt(index);}
    vertex vertAt(long index) {return _transformedMesh->vertAt(index);}
...
// intercepted methods
    ...
    {
        long topElementBeforeRemaps = TopElement(*_transformedMesh);
        _transformedMesh->consolidate(faceRemaps, edgeRemaps, vertRemaps);
        std::vector<long> elementRemaps(topElementBeforeRemaps + 1, -1);
        long i;
        for(i = 0; i != SizeL(faceRemaps); ++i)
        {
            if(faceRemaps[i] != -1)
            {
                long beforeRemap = ElementIndexForTypeAndSubIndex(bMeshElement::FACE, i);
                long afterRemap = ElementIndexForTypeAndSubIndex(bMeshElement::FACE, faceRemaps[i]);
        ...
        for(i = 0; i != SizeL(edgeRemaps); ++i)
        {
            if(edgeRemaps[i] != -1)
            {
                long beforeRemap = ElementIndexForTypeAndSubIndex(bMeshElement::EDGE, i);
                long afterRemap = ElementIndexForTypeAndSubIndex(bMeshElement::EDGE, edgeRemaps[i]);
                elementRemaps[beforeRemap] = afterRemap;
            }
        }
        for(i = 0; i != SizeL(vertRemaps); ++i)
        {
            if(vertRemaps[i] != -1)
            {
                long beforeRemap = ElementIndexForTypeAndSubIndex(bMeshElement::VERTEX, i);
                long afterRemap = ElementIndexForTypeAndSubIndex(bMeshElement::VERTEX, vertRemaps[i]);
                elementRemaps[beforeRemap] = afterRemap;
            }
        }
