...
{
    unsigned long startPosition = _bufferPosition;
...
bool
cTokenisedXMLInputStream::readToken(long tokenSize, unsigned long &result)
{
    if(_bufferPosition == _bufferSize)
    {
        return false;
...
bool
cTokenisedXMLInputStream::peekToken(long tokenSize, unsigned long &result)
{
    if(_bufferPosition == _bufferSize)
    {
        return false;
...
bool
cTokenisedXMLInputStream::readValue(long& result)
{
    if(_bufferSize < 4)
    {
        return false;
...
bool
cTokenisedXMLInputStream::readValue(unsigned long& result)
{
    if(_bufferSize < 4)
    {
        return false;
...
bool
cTokenisedXMLInputStream::skipValue(unsigned long size)
{
    if(_bufferSize < size)
    {
        return false;
...
cTokenisedXMLInputStream::cTokenisedXMLInputStream(const char* buffer, unsigned long bufferSize) :
 _buffer(buffer),
...
// prescan elements and attributes so vectors can be presized
    ...
    _bufferPosition = 0;
    long elementsSize, attributesSize;
    {
        elementsSize = -1;
        do
        {
            ...
            case 3:
                {
                    ...
                    longValue = static_cast<long>(shortValue);
                    break;
                }
            case 4:
                {
            ...
            case 6:
                {
                    ...
                    readOK = readValue(shortValue);
                    unsignedLongValue = static_cast<unsigned long>(shortValue);
                    break;
                }
            case 7:
                {
                    ...
                    readOK = readValue(charValue);
                    unsignedLongValue = static_cast<unsigned long>(charValue);
                    break;
                }
//            case 8:
