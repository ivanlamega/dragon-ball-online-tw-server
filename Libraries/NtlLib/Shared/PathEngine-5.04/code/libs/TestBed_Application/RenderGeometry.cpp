...
cLine::cLine(const float* pointArray)
{
    ...
    long bytePositionInVertexBuffer;
...
cPolygon::cPolygon(long points, const float* pointArray)
{
    assertD(points > 2);
    _points = points;
...
cText::cText(long x, long y, const char* str) :
 _x(x),
 _y(y),
 _str(str)
{
...
cSplashImage::cSplashImage(long x, long y, long width, long height, long loadedImageIndex) :
 _x(x),
 _y(y),
 _width(width),
 _height(height),
...
//    float startAdjust = -0.25f;
//    float endAdjust = 0.25f;
    float startAdjust = 0.f;
    float endAdjust = 0.f;

    data[0].position.x = _x + startAdjust;
    data[0].position.y = _y + startAdjust;
    data[0].position.z = 0.0f;

    data[1].position.x = _x + startAdjust;
    data[1].position.y = _y + _height + endAdjust;
    data[1].position.z = 0.0f;

    data[2].position.x = _x + _width + endAdjust;
    data[2].position.y = _y + _height + endAdjust;
    data[2].position.z = 0.0f;

    data[3].position.x = _x + _width + endAdjust;
    data[3].position.y = _y + startAdjust;
    data[3].position.z = 0.0f;
