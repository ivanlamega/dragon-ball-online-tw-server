...
float
cPath::getAdvanceInfo(float distance,
                           bool& reachesNextPoint,
                           bool& exactlyOnLine,
                           tPoint& point1, tPoint& point2)
{
    assertD(_size > 1);
    tLine line(PointForPosition(_originalStart), PointForPosition(position(1)));
    long length = Length(line.axis());
    distance += _distanceFromOriginalStart;
    if(static_cast<long>(distance) >= length)
    {
    ...
    ApproximatePointsAlongLine(line, length, distance, exactlyOnLine, point1, point2);
    return 0.f;
}
        ...
        )
{
    ...
    // this is now used for a persistent directional bias
