...
static long
FindTopValue(const std::vector<long>& v)
{
    long result = -1;
    for(long i = 0; i != SizeL(v); ++i)
    {
        if(v[i] > result)
        {
            result = v[i];
...
cSurfaceTypeCosts::cSurfaceTypeCosts(const std::vector<long>* surfaceTypesVector, long topSurfaceType) :
...
cSurfaceTypeCosts::cSurfaceTypeCosts(const std::vector<long>* surfaceTypesVector) :
 _surfaceTypesVector(surfaceTypesVector),
...
void
cSurfaceTypeCosts::setCost(long surfaceType, float value)
{
...
void
cSurfaceTypeCosts::setCostDirection(long surfaceType, const tPoint& value)
{
    if(surfaceType > _topSurfaceType)
    {
        return;
...
cSurfaceTypeCostsLoader::element(const char* el, const char *const* attributes)
{
    ...
    long surfaceType = ValueAsLong(attributes, "surfaceType");
