...
void
cFacesContainedByBoundary::processLoop(const std::vector<tEdge>& boundary, long beginI)
{
...
cFacesContainedByBoundary::edgeCrossed(std::vector<tEdge>& boundary, tEdge e)
{
    ...
    // crossed back over an edge after looping around other edges
        long beginI = GetIndexForEdgeEncountered(boundary, e.twin());
...
cFacesContainedByBoundary::cFacesContainedByBoundary(long edgesInMesh) :
