...
void
bCircuitElementTraversal::init_AsCloneOfPart(const bCircuitElementTraversal& toClone, long firstFace, long lastFace)
{
    assertD(cEdgesTraversal::empty());
    _line = toClone._line;
    cEdgesTraversal::init_InFace(toClone.getFaceCrossed(firstFace));
    long i = firstFace;
    while(i != lastFace)
    {
        pushEdgeCrossing(toClone.getEdgeCrossed(i));
        i++;
    }
}
void
bCircuitElementTraversal::init_ForAfterSplit(const bCircuitElementTraversal& toClone, long firstFace)
{
    assertD(cEdgesTraversal::empty());
    _line = toClone._line;
    cEdgesTraversal::init_InFace(toClone.getFaceCrossed(firstFace));
...
bCircuitElementTraversal::isValid_SI() const
{
    ...
    }
    {
        ...
        long i;
        for(i = 0; i < getEdgesCrossed(); i++)
        {
            tEdge e = getEdgeCrossed(i);
...
void
bCircuitElementTraversal::truncate(long indexForLastFace)
{
...
bCircuitElementTraversal::normaliseLine_Internal(const tLine& lineBefore, const tLine& lineAfter)
{
    ...
    {
        start = tPoint(start.getX() + axis.getX(), start.getY() + axis.getY(), WORLD_RANGE);
    }
    while(lineBefore.side(start) == sideStart)
    {
        start = tPoint(start.getX() - axis.getX(), start.getY() - axis.getY(), WORLD_RANGE);
    ...
    {
        end = tPoint(end.getX() - axis.getX(), end.getY() - axis.getY(), WORLD_RANGE);
    }
    while(lineAfter.side(end) == sideEnd)
    {
        end = tPoint(end.getX() + axis.getX(), end.getY() + axis.getY(), WORLD_RANGE);
...
bCircuitElementTraversal::addTraversal(const bCircuitElementTraversal &element2)
{
    ...
    long i;
    for(i = 0; i < element2.getEdgesCrossed(); i++)
    {
        cEdgesTraversal::pushEdgeCrossing(element2.getEdgeCrossed(i));
    }
    if(element2.exits())
    {
...
//        }
    ...
    if(!enters())
    {
        if(getPrev() && getFaceCrossed(0) != getPrev()->getLastFaceCrossed())
        {
...
void
cCircuitElement::init_AsCloneOfPart(const cCircuitElement& toClone, long firstFace, long lastFace)
{
...
cCircuitElement::connectTo(cCircuitElement& element2)
{
    ...
    assertD(!element2.enters());
    assertD(getLastFaceCrossed() == element2.getFaceCrossed(0));
...
bool cCircuitElement::operator==(const cCircuitElement& rhs) const
{
    ...
    {
        if(getFaceCrossed(i) != rhs.getFaceCrossed(i))
        {
            return false;
        }
    }
...
void
cCircuitElement::split(long faceIndexForSplit, const tLine& intersectLine)
{
    ...
    assertD(getLastFaceCrossed() == elementAfter->getFaceCrossed(0));
}
...
{
    long i = getFaceIndexForIntersection(intersectingLine);
    if(i == -1)
    {
        return tMesh::endFace();
    }
    return getFaceCrossed(i);
}
...
cCircuitElement::isValid_SI() const
{
    ...
    if(!enters() && prev)
    {
        ...
        if(!PointIsInFace<tMesh>(getFaceCrossed(0), corner))
        {
...
cCircuitElement::elementEntersThroughThis(
            long thisElementFaceIndex,
            const cCircuitElement &testElement,
            long testElementFaceIndex,
            bool& atStartOfThis, bool& atEndOfThis,
            bool& atStartOfTest
            ) const
{
    long faces = testElement.facesCrossed();
    ...
    // we need distance to intersection
        ...
        // no crossing if the intersection occurred before the start of the test element
            ...
            long comparison =  CompareFractions(startN, startD, n, d);
            if(comparison == 0)
            {
                atStartOfTest = true;
                return true;
            ...
            ) const
{
    ...
    // we need distance to intersection
        ...
        // no crossing if the intersection occurred after the end of the test element
            ...
            tPoint::tDistance endN, endD;
            testElement.getEndFraction(endN, endD);
            long comparison = CompareFractions(endN, endD, n, d);
            if(comparison == 0)
            {
                atEndOfTest = true;
                return true;
...
cCircuitElement::elementEntersThroughThis(
            ...
            bool& atStartOfThis, bool& atEndOfThis,
            bool& atStartOfTest,
            long& thisFaceIndex, long& testFaceIndex
            ) const
{
    long faces = testElement.facesCrossed();
    for(testFaceIndex = 0; testFaceIndex < faces; testFaceIndex++)
    {
        tFace face = testElement.getFaceCrossed(testFaceIndex);
...
void
cCircuitElement::getFaceBounds(long faceIndex,
                    bool& startInclusive,
                    ...
                    ) const
{
    ...
    {
        tEdge e = getEdgeCrossed(faceIndex - 1);
    ...
    {
        tEdge e = getEdgeCrossed(faceIndex);
...
cCircuitElement::lineEntersInFace_New(
        long faceIndex, const tLine& line, bool& atStartCorner, bool& atEndCorner
        ) const
{
