...
protected:
    ...
    void init_AsCloneOfPart(const bCircuitElementTraversal& toClone, long firstFace, long lastFace);
    void init_ForAfterSplit(const bCircuitElementTraversal& toClone, long firstFace);

    bool isValid_SI() const;

    void truncate(long indexForLastFace);
    void reverse();
...
public:
    ...
    void init_AsCloneOfPart(const cCircuitElement& toClone, long firstFace, long lastFace);
...
//    static void remapConnection(cCircuitElement& e1, cCircuitElement& e2);
    ...
    void split(long faceIndexForSplit, const tLine& intersectLine);
    void join();
    void reverse();
    ...
    long getFaceIndexForIntersection(const tLine& intersectingLine) const;
    tFace getFaceForIntersection(const tLine& intersectingLine) const;
    long getFaceIndexForIntersection(const tLine& intersectingLine, bool& atCorner) const; // corner could be either start or end
    ...
    {
        return cInternalPosition(getFaceCrossed(0), cornerPoint());
    }
...
// ============= collision queries
    ...
    void getFaceBounds(long faceIndex,
                    bool& startInclusive,
