...
cTree::cTree(long topSourceElement)
{
...
//    long elements = static_cast<long>(SizeL(_transformDown));
//    long i;
//    for(i = 0; i < SizeL(_operations); i++)
//    {
//        long j;
...
void
cTree::addOperation(iOperation* operation, const std::vector<long>& sources, const std::vector<long>& results)
{
    long newOperation = SizeL(_operations);
    _operations.push_back(operation);
    _prevOperation.push_back(vector<long>());
    _prevOperation.back().resize(sources.size());
    _nextOperation.push_back(vector<long>());
    _nextOperation.back().resize(results.size());
    long i;
    for(i = 0; i < SizeL(sources); i++)
    {
        long source = sources[i];
        long lastOperation = _lastOperation[source];
        if(lastOperation < 0)
        {
        ...
        else
        {
            ...
            for(long j = 0; j != SizeL(_nextOperation[lastOperation]); ++j)
            {
...
void
cTree::remap(const vector<long>& elementRemaps, long elementsAfterRemap)
{
    long i;
    for(i = 0; i != SizeL(_firstOperation); ++i)
    {
        ...
        long beforeRemap = -2 - _firstOperation[i];
        long afterRemap = elementRemaps[beforeRemap];
        _firstOperation[i] = -2 - afterRemap;
    }
    assertD(elementRemaps.size() == _lastOperation.size());
    {
        std::vector<long> remapped_lastOperation(elementsAfterRemap);
    ...
    {
        for(long j = 0; j != SizeL(_nextOperation[i]); ++j)
        {
            if(_nextOperation[i][j] < 0)
            {
                assertD(_nextOperation[i][j] != -1);
                long beforeRemap = -2 - _nextOperation[i][j];
                long afterRemap = elementRemaps[beforeRemap];
...
long
cTree::transformDown(long element, const iPointLike& p) const
{
    long operation = _firstOperation[element];
    while(operation >= 0)
    {
...
long
cTree::transformUp(long element, const iPointLike& p) const
{
    long operation = _lastOperation[element];
    while(operation >= 0)
    {
