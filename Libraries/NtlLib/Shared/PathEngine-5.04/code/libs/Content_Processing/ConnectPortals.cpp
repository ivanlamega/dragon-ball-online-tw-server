...
ResolveCutStart(const cMesh3DPartitioning& partitioning, long* resultsBuffer,
                tMesh& originalMesh,
                tVertex v, float minZ, float maxZ,
                bool& failedBecauseOfMultipleLayers
                )
{
    tPoint p = GetVertexPoint(v);
    long segments = partitioning.queryFacesPotentiallyStabbed(p, resultsBuffer);
...
ResolveCutStartToTerrain(
        const cMesh3DPartitioning& partitioning, long* resultsBuffer,
        tMesh& originalMesh,
        tVertex v,
        ...
        )
{
    ...
    std::vector<long> resultsBuffer(partitioning.maximumResultsSize());
        ...
        for(j = 0; j < SizeL(shape._insidePortalEdges); ++j)
        {
            ...
            else
            {
                startElement = ResolveCutStart(
                        partitioning, &resultsBuffer.front(),
                        baseMesh, insideEdge.vertex(),
                        minZ, maxZ, failedBecauseOfMultipleLayers
                        );
            }
