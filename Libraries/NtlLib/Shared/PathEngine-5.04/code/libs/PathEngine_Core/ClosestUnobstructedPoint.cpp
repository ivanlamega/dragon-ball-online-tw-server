...
void
GetElementsWithinBounds(const vector<long>& facesWithinBounds,
...
public:
    ...
    long _checkAgainstBaseCircuits;
...
public:
    ...
    void add(const tIntersection& p, const vector<long>& facesToResolveTo, bool checkAgainstBaseCircuits);
    void add(const tPoint& p, const vector<long>& facesToResolveTo, bool checkAgainstBaseCircuits);
    ...
    void add(const tPoint* element1, const tPoint* element2,
             ...
             const vector<long>& facesToResolveTo,
             bool checkAgainstBaseCircuits);
};

// returns true is rhs is closer than lhs
bool
...
void
cCandidateSet::add(const tIntersection& p, const vector<long>& facesToResolveTo, bool checkAgainstBaseCircuits)
{
    long i;
...
void
cCandidateSet::add(const tPoint& p, const vector<long>& facesToResolveTo, bool checkAgainstBaseCircuits)
{
    long i;
    for(i = 0; i < SizeL(facesToResolveTo); ++i)
    {
                      ...
                      cCandidateSet& candidates)
{
    ...
    for(i = 0; i < SizeL(elements); ++i)
    {
        ...
        if(e->getPrev()->refLine().side(e->refLine()) != SIDE_RIGHT)
        {
            ...
            candidates.add(p, e->getFaceCrossed(0), false);
        }
                                          ...
                                          cCandidateSet& candidates)
{
    ...
    for(i = 0; i < SizeL(baseCircuitElements); ++i)
    {
        ...
        {
            const tPoint* element2 = shape.dataForSideElement(j);
...
//static void
//IntersectionsBetweenShapeAndSmallConvex(const cMeshCut& smallConvex,
//                                          const cLocalisedConvexShape& shape, bool isInverted,
//                                          cCandidateSet& candidates)
//{
//    const cCircuitElement* e = smallConvex.getStartElement();
...
//        tPoint element1[3];
//        bool element1HasConvexCorner;
//        CircuitElement2DData(e, element1, element1HasConvexCorner);
//        long j;
//        for(j = 0; j < shape.sides(); ++j)
//        {
//            const tPoint* element2 = shape.dataForSideElement(j);
                           ...
                           cCandidateSet& candidates)
{
    ...
    {
        const tPoint* element1 = shape1.dataForSideElement(i);
        long j;
        for(j = 0; j < shape2.sides(); ++j)
        {
            const tPoint* element2 = shape2.dataForSideElement(j);
            bool atCorner1, atCorner2;
            if(ElementsPotentiallyCreateConcaveCorner(
...
ShapeNormalIntersections(const cLocalisedConvexShape& shape, const tPoint& centre, cCandidateSet& candidates)
{
    ...
    {
        tLine boundaryLine = shape.getBoundaryLine(i);
        ...
        )
{
  // initially empty set of geometry that has been added
    ...
    vector<long> staticExpansionsAdded;
    vector<const cLocalisedConvexShape*> dynamicExpansionsAdded;

    cLocalisedConvexShape bounds(qc, centre, radius);
