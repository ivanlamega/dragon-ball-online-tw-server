                        ...
                        )
{
    ...
    tFace f = testElement.getFaceCrossed(0);
    ...
    for(i = baseCuts.begin(); i != baseCuts.end(); ++i)
    {
        ...
        do
        {
            ...
            if(baseElement->getFaceCrossed(0) != f)
            {
                continue;
            }
                    ...
                    )
{
    ...
    for(elementFaceIndex = 0; elementFaceIndex < faces; elementFaceIndex++)
    {
        tFace f = element.getFaceCrossed(elementFaceIndex);
...
void
CheckNoSelfIntersections(long facesInMesh, long edgesInMesh, const tMeshCutSequence& cuts)
{
...
// check for any contained existing circuits

    {
        ...
        {
            long faceIndex = facesOverlapped[j];
