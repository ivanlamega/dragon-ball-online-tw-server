...
class cTranslationToOverlapped
{
    ...
    std::vector<std::vector<long> > _facesOverlappedBy2D;
    std::vector<std::vector<long> > _facesOverlappedBy3D;
...
public:
    ...
    tFace_3D translate(tFace f, tPointLike& p) const
    {
        if(!_bsps.empty())
        {
            long resultIndex = _bsps[f.index()]->query(p);
    ...
    long numberOfFacesOverlapped(tFace f) const;
    long numberOfFacesOverlapped(long f2DIndex) const;
    long iterateInSetOfFacesOverlapped(long f2DIndex, long i) const;
};
