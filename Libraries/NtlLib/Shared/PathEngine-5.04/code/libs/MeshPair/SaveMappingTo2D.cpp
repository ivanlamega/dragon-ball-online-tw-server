...
SaveMappingTo2D(long verticesIn3D, tMesh& mesh, iXMLOutputStream& os)
{
    ...
    std::vector<long> targetIndex(mesh.vertices());
    {
        long numberOfAdditionalVertices = 0;
        for(i = 0; i != mesh.vertices(); ++i)
        {
            long elementIndex = mesh.vertAt(i)->getElementOn3DIndex();
    ...
    while(1)
    {
        ...
        do
        {
            ...
            long connection = e.twin().face().index();
            if(connection < i)
            {
