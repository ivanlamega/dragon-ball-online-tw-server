...
// follow 2d edge lines through 3d mesh
    ...
    {
        long elementIndex = e.vertex()->getElementOn3DIndex();
    ...
    do
    {
        ...
        {
            long spinCount = SpinCountAroundVertex(e);
...
{
    long internalFacesOn2D = CountInternalFaces(*_mesh2D);
...
// generate table of faces overlapped by mesh
    ...
    do
    {
        if(f->isExternal())
        {
            long vertex3D = tMesh::beginEdge(f).vertex()->getTargetVertexIndex();
...
// reverse this table for mesh_3d
    ...
    for(i = 0; i < SizeL(_facesOverlappedBy2D); i++)
    {
        ...
        {
            long overlapped = _facesOverlappedBy2D[i][j];
...
// build bsps if requested
    ...
    std::vector<long> buffer(_mesh3D->edges(), 0);
...
cTranslationToOverlapped::translateEntry(tEdge_3D entryEdge, const tLine& entryLine) const
{
    ...
    long fIndex = entryEdge.face().index();
    long i;
...
long
cTranslationToOverlapped::numberOfFacesOverlapped(long f2DIndex) const
{
    return SizeL(_facesOverlappedBy2D[f2DIndex]);
}
long
cTranslationToOverlapped::iterateInSetOfFacesOverlapped(long f2DIndex, long i) const
{
