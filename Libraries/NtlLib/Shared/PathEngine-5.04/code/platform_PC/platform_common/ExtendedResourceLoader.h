...
public:
    ...
    void saveResource(const char* filename, const char* buffer, unsigned long bufferSize)
    {
        std::string fullFileName = "..\\resource\\";
        fullFileName += filename;
        SaveBinary(fullFileName.c_str(), buffer, bufferSize);
    }
    void getDirectoryListing(const char* path, const char*& listingBuffer, unsigned long& listingSize)
    {
