...
static void
SelectFaces(MDagPath path, const std::vector<long>& indices)
{
    MStatus status;
    ...
    {
        long faceIndex = it.index();
        for(long i = 0; i != SizeL(indices); ++i)
        {
...
static void
SelectVertices(MDagPath path, const std::vector<long>& indices)
{
    MStatus status;
...
void
cMayaErrorHandler::setContext(const std::string& description, std::vector<MDagPath>* dagPaths)
{
...
cMayaErrorHandler::handle(const char* errorType, const char* errorString, const char *const* attributes)
{
    ...
    {
        long partIndex = -1;
        std::vector<long> faceIndicesInPart;
        std::vector<long> vertexIndicesInPart;
