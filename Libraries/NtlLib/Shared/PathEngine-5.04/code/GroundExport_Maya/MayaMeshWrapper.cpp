...
ColourMatchesAShadeOfGreen(const MColor& colour, int& shadeOfGreen)
{
    ...
    assertD(shadeOfGreen > 0 && shade <= 255);
    return true;
}
...
void
cMayaMeshWrapper::extractAttributesFromMaterial(const MFnDependencyNode& material, long faceIndex)
{
    MStatus status;
    ...
    char* afterParsing;
    long surfaceType = strtol(notes + valueStart, &afterParsing, 10);
...
cMayaMeshWrapper::extractAttributesFromShaders(const MObjectArray& shaders, const MIntArray& indices)
{
    ...
    for(long i = 0; i < faces(); ++i)
    {
        if(static_cast<unsigned long>(i) >= indices.length())
        {
            break;
        }
...
cMayaMeshWrapper::cMayaMeshWrapper(const MDagPath& dagPath, float scale)
{
    ...
    {
        _transformedVertexXY[i * 2 + 0] = static_cast<long>(vertexArray[i].z * scale);
        _transformedVertexXY[i * 2 + 1] = static_cast<long>(vertexArray[i].x * scale);
...
{
	return static_cast<long>(_mesh->numPolygons());
}
long
cMayaMeshWrapper::vertices() const
{
	return static_cast<long>(_mesh->numVertices());
}

long
cMayaMeshWrapper::vertexIndex(long face, long vertexInFace) const
{
...
long
cMayaMeshWrapper::vertexX(long vertex) const
{
    return _transformedVertexXY[vertex * 2 + 0];
}
long
cMayaMeshWrapper::vertexY(long vertex) const
{
    return _transformedVertexXY[vertex * 2 + 1];
}
float
cMayaMeshWrapper::vertexZ(long vertex) const
{
    return _transformedVertexZ[vertex];
}
long
cMayaMeshWrapper::faceAttribute(long face, long attributeIndex) const
{
