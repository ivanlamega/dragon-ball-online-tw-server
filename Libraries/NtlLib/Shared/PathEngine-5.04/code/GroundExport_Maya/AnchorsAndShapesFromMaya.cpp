...
ProcessShape(const MDagPath& transformDagPath, cAnchorsAndPinnedShapes& toBuild, const char* name, const tPoint& origin, float scale, bool asNamedObstacle)
{
    ...
    for(int i = 0; i < numSpans; i++)
    {
        ...
        assertR(status == MStatus::kSuccess);
        addTo.back()._coords[i * 2 + 0] = static_cast<long>(p.z * scale) - origin.getX();
        addTo.back()._coords[i * 2 + 1] = static_cast<long>(p.x * scale) - origin.getY();
    }
}
...
//    }
    ...
    tPoint origin;
    {
        long originX = static_cast<long>(points[1].z * scale);
        long originY = static_cast<long>(points[1].x * scale);
