...
static void
ProcessShape(Interface* ip, cAnchorsAndPinnedShapes& toBuild, INode* node, const char* name, const Point3& origin, bool asNamedObstacle, float scale)
{
...
static void
ProcessAnchor(Interface* ip, cAnchorsAndPinnedShapes& toBuild, INode* node, const char* name, float scale, bool resolvesToTerrain)
{
    TimeValue time = ip->GetTime();
    ...
    for (int c = 0; c < node->NumberOfChildren(); c++)
    {
        ...
        {
            ProcessShape(ip, toBuild, child, name + 6, origin, false, scale);
        }
        else
        if(strncmp(name, "baseObstacle_", 13) == 0)
        {
            ProcessShape(ip, toBuild, child, name + 13, origin, false, scale);
        }
        else
        if(strncmp(name, "obstacle_", 9) == 0)
        {
            ProcessShape(ip, toBuild, child, name + 9, origin, true, scale);
        }
	}
    ...
    long orientationL = static_cast<long>(orientationAtan2 * multiplier);
    long orientationPathEngine = ((262144 - orientationL) & 1048575);
    toBuild._anchors.back()._orientation = orientationPathEngine;
}

static void
BuildAnchorsAndShapes_Recursive(Interface* ip,
                                ...
                                float scale)
{
    ...
    if(strncmp(name, prefix, prefixLength) == 0)
    {
	    for(int c = 0; c < node->NumberOfChildren(); c++)
        {
            ...
            {
                ProcessAnchor(ip, toBuild, child, name + 7, scale, false);
            }
            else if(strncmp(name, "terrainAnchor_", 14) == 0)
            {
                ProcessAnchor(ip, toBuild, child, name + 14, scale, true);
            }
        }
    }
    long i;
...
void
BuildAnchorsAndShapesFromMax(Interface* ip, const char* prefix, cAnchorsAndPinnedShapes& toBuild, float scale)
{
    if(prefix)
    {
        long prefixLength = static_cast<long>(strlen(prefix));
...
static void
BuildAnchorsAndShapes_AllInScene_Recursive(Interface* ip,
                                cAnchorsAndPinnedShapes& toBuild,
                                INode* node,
                                float scale)
{
    ...
    if(strncmp(name, "anchor_", 7) == 0)
    {
        ProcessAnchor(ip, toBuild, node, name + 7, scale, false);
    }
    else if(strncmp(name, "terrainAnchor_", 14) == 0)
    {
        ProcessAnchor(ip, toBuild, node, name + 14, scale, true);
    }
    long i;
    for(i = 0; i < node->NumberOfChildren(); ++i)
    {
...
void
BuildAnchorsAndShapesFromMax_AllInScene(Interface* ip, cAnchorsAndPinnedShapes& toBuild, float scale)
{
    INode* root = ip->GetRootNode();
    BuildAnchorsAndShapes_AllInScene_Recursive(ip,
                toBuild,
