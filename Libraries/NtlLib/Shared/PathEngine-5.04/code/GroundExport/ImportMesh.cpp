        ...
        )
{
    ...
    if(vertexColoursRequired)
    {
        ...
        for(i = 0; i < mesh.faces(); ++i)
        {
            ...
            long sectionID = mesh.faceAttribute(i, PE_FaceAttribute_SectionID);
            if(sectionID >= 0 && sectionID <= 254)
            {
                float g = 1.f - static_cast<float>(sectionID) / 255.f;
                SetFaceColours(maxMesh, i, 0.f, g, 0.f);
            }
            else if(mesh.faceAttribute(i, PE_FaceAttribute_MarksPortal) == 1)
            {
...
CreateAnchor(
        ...
        long orientation, float orientationIndicatorLength,
        float scale,
...
static INode*
CreateShapePolygon(Interface* ip, const std::vector<tPoint>& points, float height, float scale, const char* baseName, bool makeNameUnique)
{
    LinearShape* linearShape = new LinearShape();
    assertD(linearShape);
    Object* obj = linearShape;
    ...
    {
        PolyPt pp;
        pp.p.x = points[i].getX();
        pp.p.y = points[i].getY();
        pp.p.z = height;
        pp.p /= scale;
        line->Append(pp);
    }
...
AveragePoint(const std::vector<tPoint>& points)
{
    ...
    return tPoint(static_cast<long>(x / divisor), static_cast<long>(y / divisor), WORLD_RANGE);
}

void
ImportBaseObstacle(
        Interface* ip, INode* parent,
        const std::vector<tPoint>& points, float height, float heightRange, float scale
        )
{
        ...
        )
{
    ...
    for(i = 0; i != anchorsAndShapes.numberOfAnchors(); ++i)
    {
        ...
        long orientation = anchorsAndShapes.anchor_Orientation(i);
    ...
    {
        long anchorIndex = anchorsAndShapes.pinnedShape_AnchorIndex(i);
        long vertices = anchorsAndShapes.pinnedShape_NumberOfVertices(i);
        std::vector<tPoint> points(vertices);
        long x, y;
        anchorsAndShapes.anchor_HorizontalPosition(anchorIndex, x, y);
        ...
        for(j = 0; j != vertices; ++j)
        {
            ...
            points[j] = centre + tPoint(x, y, SHAPE_RANGE);
        }
        float zStart, zEnd;
