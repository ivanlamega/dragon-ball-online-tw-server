...
inline char*
LoadBinary_AllocatedBuffer(const char* fileName, unsigned long& size)
{
    size = 0;
    FILE* fp = fopen(fileName, "rb");
    if(!fp)
...
inline bool
SaveBinary(const char* fileName, const char* buffer, unsigned long bufferSize)
{
    ...
    if(bufferSize > 0)
    {
        long amountWritten = static_cast<long>(fwrite(buffer, 1, bufferSize, fp));
...
SaveBinary(const char* fileName, const std::vector<char>& buffer)
{
    if(buffer.empty())
    {
        return SaveBinary(fileName, 0, static_cast<unsigned long>(buffer.size()));
    }
    return SaveBinary(fileName, &buffer.front(), static_cast<unsigned long>(buffer.size()));
}
