...
static INode*
CreateShapePolygon(Interface* ip, const std::vector<long>& points, float height, float scale, const char* baseName, bool makeNameUnique)
{
    LinearShape* linearShape = new LinearShape();
    assertD(linearShape);
    Object* obj = linearShape;
...
ImportBaseObstacle(
        ...
        const std::vector<long>& points, float height, float heightRange, float scale
        )
{
        ...
        )
{
    ...
    for(i = 0; i != anchorsAndShapes.numberOfPinnedShapes(); ++i)
    {
        ...
        std::vector<long> points(vertices * 2);
