...
void
cScene::decomposeCurrentlySelected(bool& isMeshObject, long& vectorIndex) const
{
...
iMesh*
cScene::buildGround(long extrudeHeight, long maxStepHeight, long maxSlope)
{
    std::vector<const iFaceVertexMesh*> meshInstances;
    std::vector<long> solidObjectsData;
    long i;
    for(i = 0; i != SizeL(_meshObjects); ++i)
    {
    ...
    for(i = 0; i != SizeL(_solidObjects); ++i)
    {
        _solidObjects[i]->addTransformedData(solidObjectsData);
    }
    solidObjectsData.push_back(0);
    cGIPProgressCallBack progressCallBack;
    return BuildMeshFrom3DContent(
        VectorBuffer(meshInstances), SizeL(meshInstances),
        &solidObjectsData[0], SizeL(solidObjectsData),
        extrudeHeight,
        maxStepHeight,
        maxSlope,
        0, // no options
