...
Demo()
{
    const long maxStepHeight = 500; // allow maximum 50 centimetres step height
    const long maxSlope = 65; // allow maximum 65% gradient
    ...
    cScene scene;
    {
        ...
        bool parsedOK = ParseXML(&buffer[0], static_cast<unsigned long>(buffer.size()), scene);
        if(!parsedOK)
        {
