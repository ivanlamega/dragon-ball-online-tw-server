...
public:
    ...
    iMeshFederation*
    buildMeshFederation_FromWorldMesh(const iMesh* worldMesh, long tileSize, long overlap, iMeshFederationTileGeneratedCallBack& tileGeneratedCallBack);

    iMeshFederation*
    loadFederation(const char* format, const char* dataBuffer, unsigned long bufferSize);

    iMeshFederation*
    buildMeshFederation_TilingOnly(const cHorizontalRange& worldRange, long tileSize, long overlap);
};
