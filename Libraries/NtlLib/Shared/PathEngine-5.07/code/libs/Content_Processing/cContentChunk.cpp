  ...
  // i.e. the set of ground edges that share vertex pairs with connecting edge marker faces
    ...
    if(_ground)
    {
      ...
      // and store indices for next vertex around the marker
        ...
        std::vector<long> connectingEdgeNext(_ground->vertices() * 2, -1);
        for(i = 0; i != _ground->faces(); ++i)
        {
            ...
            {
                long nextJ = (j + 1) % 3;
                long v = _ground->vertexIndex(i, j);
                long nextV = _ground->vertexIndex(i, nextJ);
...
void
cContentChunk::getConnectingEdgeInfo(long connectingEdgeIndex, long& face, long& firstVertexInFace) const
{
...
void
cContentChunk::releaseInstance(long index)
{
    cMutexLock lock(_mutex);
...
cContentChunk::instantiate(long rotation, long scale, long translationX, long translationY, float translationZ, long sectionID)
{
    ...
    cMutexLock lock(_mutex);
    long allocatedIndex = _instances.reuseOrPushBack(0);
...
iContentChunkInstance*
cContentChunk::instantiate_FloatRotateAndScale(float rotation, float scale, long translationX, long translationY, float translationZ, long sectionID)
{
    iFaceVertexMesh* faceVertex = 0;
    iAnchorsAndPinnedShapes* anchorsAndShapes = 0;
    if(_ground)
