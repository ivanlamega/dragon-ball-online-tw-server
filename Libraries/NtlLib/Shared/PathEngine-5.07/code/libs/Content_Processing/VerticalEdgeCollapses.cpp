...
bool
cVerticalEdgeCollapses::collapseEdgeIfVertical(const iFaceVertexMesh& mesh, long v0, long v1)
{
 ...
 _zRanges(mesh.vertices() * 2)
{
  ...
  // propagate collapses down
    for(i = 0; i < mesh.vertices(); i++)
    {
        long propagatedCollapse = _vertexCollapseTable[i];
  ...
  // fill in z ranges
    for(i = 0; i < mesh.vertices(); i++)
    {
        long collapsed = _vertexCollapseTable[i];
