...
bool ParseXML(const char* buffer, unsigned long bufferSize, iXMLHandler& handler)
{
    bool parsed;
...
static bool ReadString(
                ...
                std::string& result)
{
    unsigned long start_position = bufferPosition;
...
{
    if(bufferSize - bufferPosition < static_cast<unsigned long>(tokenSize))
    {
        return false;
    }
...
bool
ParseTokenisedXML(const char* buffer, unsigned long bufferSize, iXMLHandler& handler)
{
...
bool
ParseXML(const char* format, const char* buffer, unsigned long bufferSize, iXMLHandler& handler)
{
