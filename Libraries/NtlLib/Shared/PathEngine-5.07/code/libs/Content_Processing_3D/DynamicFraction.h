...
public:
    ...
    cDynamicFraction(const cDynamicInteger di) :
     numerator(di),
     denominator((unsigned long)(!numerator.isZero()))
	{
	}
    cDynamicFraction(const long l) :
     numerator(l),
     denominator((unsigned long)(l != 0))
	{
	}
...
//		return !(s1>s2);
	...
	long asLong_RoundToNearest() const
	{
		if(numerator.isZero())
        {
			return 0;
