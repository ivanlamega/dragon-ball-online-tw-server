...
public:
    cVertexIsBehindPredicate(const cBitVector* sidesBuffer, long sidesStart, long vertsBeforeSplit) :
...
SideForEdgeStart(
        tEdge_Dynamic e, 
        const cBitVector& sidesBuffer, long sidesStart, long verticesBeforeSplit
        )
{
    long i = e.vertex().index();
...
static void
AddInsideFace(tMesh_Dynamic& mesh, tEdge_Dynamic edgeOnBoundary, long splitNode, bool facesOppositeDirection)
{
...
cBSP3DOperand::cBSP3DOperand(
        cBSPVertices& vertices,
        tMesh_Simple3D& meshSimple3D, long operandIndex, std::vector<cPlane>& splitters
        )
{
        ...
        )
{
    ...
    for(long i = 0; i != _mesh.vertices(); ++i)
    {
        ...
        )
{
    ...
    do
    {
        ...
        if(sideAtStart == SIDE_CENTRE)
        {
            ...
            if(sideBefore == SIDE_CENTRE && sideAfter == SIDE_CENTRE)
            {
              // coplanar face
                ...
                assertD(sideForOperand != SIDE_CENTRE);
            ...
            if(sideBefore == SIDE_CENTRE || sideAfter == SIDE_CENTRE || sideBefore == sideAfter)
            {
        ...
        eSideOfPlane sideAfter = RetrieveSideOfPlane(preprocessBuffer, bufferPosition, e.next().vertex().index());
        if(sideAfter == SIDE_CENTRE || sideAfter == sideAtStart)
        {
            // touches split
            if(sideAtStart == IN_FRONT)
            {
        ...
        )
{
...
#ifdef ASSERTIONS_ON
    long predictedUnassignedInFront;
    long predictedUnassignedBehind;
    ...
    long sidesStart = bufferPosition;
    long vertsBeforeSplit = _mesh.vertices();
...

    {
        ...
        long facesBeforeSplit = _mesh.faces();
        long i;
...
long
cBSP3DOperand::splitForUnassignedBoundIndex(long index) const
{
    tFace_Dynamic f = _mesh.beginFace();
    do
    {
