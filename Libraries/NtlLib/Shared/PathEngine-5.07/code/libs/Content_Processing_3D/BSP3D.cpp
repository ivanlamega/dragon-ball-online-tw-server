...
{
    std::vector<long> splitterRemaps(SizeL(splitters));
    {
        long i;
        for(i = 0; i != SizeL(splitters); ++i)
        {
...

    {
        ...
        long toProcess = result.vertSize();
        do
        {
            if(progressCallBack)
            {
        ...
        )
{
    ...
    Simplify_PostBSP(manager._vertices, result, manager._splitters, projectionTypes, progressCallBack);
        ...
        )
{
    ...
    std::vector<int> projectionTypes(manager._splitters.size());
    GetSplitterProjectionTypes(manager._splitters, projectionTypes);
    Simplify_PostBSP(vertices, result, manager._splitters, projectionTypes, progressCallBack);
    RemoveDownwardFacingFaces(result, manager._splitters);
    RemoveFacesAboveMaxSlope(result, manager._splitters, maxSlopeFraction, progressCallBack);
    if(result.empty())
    {
