...
static long
SplitForUnassignedBoundIndex(const std::vector<cBSP3DOperand*>& operands, long boundIndex)
{
    long i;
    for(i = 0; i != SizeL(operands); ++i)
    {
        long bounds = operands[i]->numberOfUnassignedBounds();
        ...
        )
{
    ...
    long i = manager._result.faces();
    long facesToProcess = i;
    while(i)
    {
        --i;
        ...
        long node = toSplit->getSplitNode(facesOppositeDirection);;
        ...
        )
{
    ...
    long preprocessPosition = 0;
    long frontUnassigned = 0;
    long backUnassigned = 0;
//    long totalCost = 0;
    bool frontIsContained = false;
    bool backIsContained = false;
    for(long i = 0; i != SizeL(operands); i++)
    {
        long frontUnassignedInOperand, backUnassignedInOperand;
        ...
        )
{
    ...
    long splitterSelected = _bestSplit;
    _bestSplit = -1;

    frontContained = false;
    backContained = false;
    long operandContainingFront;
    long operandContainingBack;

#ifdef ASSERTIONS_ON
    long unassignedBoundsElsewhere = _totalUnassignedBounds;
#endif
    ...
    for(i = 0; i != SizeL(operands); i++)
    {
        ...
        long nextBufferPosition = bufferPosition + operands[i]->numberOfVertices() * 2;
        operands[i]->split(
                _vertices,
    ...
    if(_progressCallBack)
    {
        ...
        long dispatched = _totalStartingUnassignedBounds - _bestUnassignedBounds;
...
{
    long operandIndex = SizeL(_initialOperands);
        ...
        )
{
    ...
    long totalSplits = CountUnassignedBounds(operands);
...
//    splitsToTry = 1; //.......
    ...
    std::vector<long> tried(splitsToTry);

    long i;
    for(i = 0; i != splitsToTry; ++i)
    {
        long splitAsBoundIndex = _sequence.random31() % (totalSplits - i);
        ...
        long splitterIndex = SplitForUnassignedBoundIndex(operands, splitAsBoundIndex);
