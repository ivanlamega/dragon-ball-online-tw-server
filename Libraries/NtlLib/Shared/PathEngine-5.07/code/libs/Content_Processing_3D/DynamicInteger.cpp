...
cDynamicInteger cDynamicInteger::operator+= (const cDynamicInteger &di2)
{
	...
	cDigitBlock *destptr;
	unsigned long srcendindex,destendindex;
	unsigned long i;
	unsigned long afteraddition;
...
{
    unsigned long digit1 = lastblockptr->digitarray[topdigit & DIGITSPERBLOCKMASK];
    float digit1F = static_cast<float>(digit1);
    float result = digit1F;
    if(topdigit > 0)
    {
        result *= 4294967296.f;
        unsigned long digitsLeft = topdigit - 1;

        unsigned long digit2Index = topdigit - 1;
        unsigned long digit2;
...
{
	cDigitBlock *ptr;
	unsigned long shiftup;
	unsigned long nextshiftup;
	int i;
	int endi;
	if(isZero())
		return;
...
void cDynamicInteger::initialiseFrom(unsigned long firstdigit, unsigned long seconddigit, unsigned long topdigit)
{
...
//	profiler aprofiler(&aprofile);
	...
	unsigned long src1endindex,src2endindex;
	unsigned long src1index,src2index;

	unsigned long destendindex;
	cDigitBlock *destptr;
	cDigitBlock *destptr2;
	unsigned long destindex;
	unsigned long destindex2;

	unsigned long multiplier;
	unsigned long product;
	unsigned long producthigh;
	unsigned long carry;
	unsigned long carryhigh;
	unsigned long nextcarryhigh;
...
bool
cDynamicInteger::cansubtractresidueproduct(const cDynamicInteger &divisor, unsigned long partialquotient, unsigned long partialremainder) const
{
...
//.. optimise - dont do subtraction twice ?
	...
	cDigitBlock *destptr;
	const cDigitBlock *srcptr;
	unsigned long destindex,srcindex;
	unsigned long producthigh,productlow;
	...
	int carrypermitted;
	unsigned long subtractfrom;
	unsigned long previousproductlow;
	unsigned long beforesubtract;
...
void cDynamicInteger::subtractresidueproduct(const cDynamicInteger &divisor, unsigned long partialquotient, unsigned long partialremainderhigh, unsigned long partialremainderlow)
{
...
//.. optimise - dont do subtraction twice ?
	...
	unsigned long destendindex;
	unsigned long previousproducthigh;
	int carry;
	unsigned long producthigh,productlow;
	unsigned long beforesubtraction;
...
// absolute values of di and *this
	...
	unsigned long divisortop;
	cDynamicInteger quotient;
	unsigned long partialremainder;
	unsigned long partialremainderhigh;
	unsigned long topdigitbefore;
	unsigned long digitsretired;
	unsigned long i;
	unsigned long digitstoretire;
	int bitstoretire;
	unsigned long dividendtophigh;
	unsigned long dividendtoplow;
	unsigned long partialquotient;
	...
	if(!absoluteValueGreaterThan(*this,di))
	{
		if(!absoluteValueGreaterThan(di,*this))
		{
			...
			initialiseFrom((unsigned long)1);
...
public:
	...
	unsigned long &digit(cDigitBlock *const ptr) const
	{
		return ptr->digitarray[index];
	}
...
//		}
	...
	void getbitsleft(unsigned long &digits, int &bits) const
	{
...
// remove remaining trailing zeros from whichever has any
	...
	// determine number of extra digits and bits in large value
		unsigned long digitsleft1,digitsleft2;
...
////VtResumeSampling();
//
//	cDynamicInteger result((unsigned long)1);
