...
ConvexHull3D_BasePyramidPoints(
        long points, const tPoint3* pointArray,
        tPoint3* pyramid,
...
//            if(tPoint3::tMultiplier::isZero(dp))
            if(dp.isZero())
            {
                ...
                dp = normal.dotProduct(offset);
//                assertD(!tPoint3::tMultiplier::isZero(dp));
                assertD(!dp.isZero());
            }
        }
...
//    if(!tPoint3::tMultiplier::isNegative(dp))
    if(!dp.isNegative())
    {
...
void
ConvexHull3D(long points, const tPoint3* pointArray, tMesh_Simple3D& result)
{
    std::list<tPoint3> remainingPoints;
