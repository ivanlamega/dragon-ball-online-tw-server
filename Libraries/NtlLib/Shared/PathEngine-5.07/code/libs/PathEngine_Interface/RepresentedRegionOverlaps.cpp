...
{
    for(long i = 0; i != subsetsLookup.size(); ++i)
    {
        const long *const subVector = subsetsLookup[i];
        long j;
...
cRepresentedRegionOverlaps::save(iXMLOutputStream& os) const
{
    ...
    for(long i = 0; i != _overlaps.size(); ++i)
    {
        ...
        for(long j = 0; j != _overlaps.subVectorSize(i); j += 2)
        {
