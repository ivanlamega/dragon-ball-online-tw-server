...
cObstacleSet::cObstacleSet(cMesh* mesh, const char *const* attributes) :
 _unexpandedPreprocess(0),
 _collisionPreprocess(nSingletons::pathEngine().numberOfPreprocessShapes(), 0),
...
void
cObstacleSet::setIndexInMesh(long value)
{
...
cObstacleSet::removeAgent(iAgent *agent)
{
    ...
    _mesh->lockAgentsAndObstacleSets();
    cAgent* a = static_cast<cAgent*>(agent);
    long indexInAgentVector = a->notifyRemovedFromObstacleSet(_indexInMesh);
...
iAgent*
cObstacleSet::getAgent(long index) const
{
    cMutexLock lock(_mesh->refAgentsAndObstacleSetsMutex());
...
{
    cMutexLock lock(_mesh->refAgentsAndObstacleSetsMutex());
    long result = SizeL(_agentVector);
    return result;
}
...
cHandle<cContextCollisionPreprocess>
cObstacleSet::getCollisionPreprocessFor(cQueryContext& qc, long i) const
{
    cMutexLock lock(_mesh->refAgentsAndObstacleSetsMutex());
    {
        cMutexLock lock(_preprocessMutex);
...
cObstacleSet::getPathfindPreprocessFor(cQueryContext& qc, long i) const
{
    ...
    cMutexLock lock(_mesh->refAgentsAndObstacleSetsMutex());
    {
        ...
        const std::vector<cCollisionShape>& baseObstacleShapes = _mesh->refBaseObstacleShapes();
        const cCollisionShape& expandShape = nSingletons::pathEngine().refPreprocessShape(i);
        ...
        for(j = 0; j != SizeL(baseObstacles); ++j)
        {
            ...
            cLocalisedConvexShape* lcs = new cLocalisedConvexShape(qc, baseObstacleShapes[j], info._position, expandShape);
...
void
cObstacleSet::addCollisionPreprocessFor(cQueryContext& qc, long index, cCompoundContextCollisionPreprocess& addTo)
{
    cMutexLock lock(_mesh->refAgentsAndObstacleSetsMutex());
...
bool
cObstacleSet::pathfindingStateHasBeenModifiedSince(unsigned long stateHandle) const
{
