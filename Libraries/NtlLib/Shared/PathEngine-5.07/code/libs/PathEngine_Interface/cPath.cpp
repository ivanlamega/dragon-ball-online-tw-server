        ...
        )
{
    ...
    if(static_cast<long>(distance) >= _segmentInterpolator.segmentLength())
    {
    ...
    long interpolatorValue = static_cast<long>(distance * _segmentInterpolator.interpolatorMax()) / _segmentInterpolator.segmentLength();
...
//    long originalLineLength = Length(originalLine.axis());
//    float updatedDistance = distance + _distanceFromOriginalStart;
//    if(static_cast<long>(updatedDistance) >= originalLineLength)
//    {
//        reachesNextPoint = true;
...
//    long currentLineLength = Length(currentLine.axis());
//    if(static_cast<long>(distance) >= currentLineLength)
//    {
...
//    _length -= Length(_internalPositions[_startOffset+1].p - p);
...
void
cPath::init(std::vector<cInternalPosition>& points, std::vector<long>* connectionIndices)
{
...
void
cPath::init(const cPosition* positionsBuffer, const long* connectionIndicesBuffer, long pathSize)
{
    assertD(pathSize > 0);
    assertD(_mesh);
    ...
    {
        assertD(pathSize > 1);
        _connectionIndices = new std::vector<long>(pathSize - 1);
        long i;
...
void
cPath::init_Reversed(const cPosition* positionsBuffer, const long* connectionIndicesBuffer, long pathSize)
{
    assertD(pathSize > 0);
    assertD(_mesh);
    if(connectionIndicesBuffer)
...
//        _toggleApproximationDirection = ValueAsBoolWithDefault(rootAttributes, "toggleApproximation", false);
    ...
    {
        _connectionIndices = new std::vector<long>(_size - 1);
        for(i = 0; i + 1 < _size; ++i)
        {
...
cPosition
cPath::position(long pointIndex) const
{
    cMutexLock lock(_mutex);
    return _points[_startOffset + pointIndex];
}
long
cPath::connectionIndex(long segmentIndex) const
{
    cMutexLock lock(_mutex);
...
cPath::getLength() const
{
    ...
    return static_cast<unsigned long>(_length - _distanceFromOriginalStart);
}
...
void
cPath::renderOnGround(long originX, long originY, iRender3DLinesCallBack& callBack) const
{
    cMutexLock lock(_mutex);
    assertD(_mesh);
    long i;
...
//void
//cPath::replaceEnd(long replaceFromIndex, const std::list<std::pair<tFace, tPoint> >& pathToAppend)
//{
...
//void
//cPath::replaceStart(long replaceToIndex, const std::list<std::pair<tFace, tPoint> >& pathToPrepend)
//{
...
//    assertD(size() >= 2);
//    long result = size() - 2;
//    long index = _startOffset + result;
//    while(result > 0)
//    {
            ...
            )
{
    ...
    vector<long>* connectionIndicesResult = 0;
    GenerateCurvedPath(
            _mesh, shape, context,
