                       ...
                       const char *const* options)
{
    ...
    long numberOfTerrainLayers = ValueAsLongWithDefault(options, "numberOfTerrainLayers", 1);
...
iMeshFederation*
cPathEngine::buildMeshFederation_FromWorldMesh(const iMesh* worldMesh, long tileSize, long overlap, iMeshFederationTileGeneratedCallBack& tileGeneratedCallBack)
{
...
iMeshFederation*
cPathEngine::loadFederation(const char* format, const char* dataBuffer, unsigned long bufferSize)
{
    cTokenisedXMLInputStream is(dataBuffer, bufferSize);
    cMutexLock lock(*_heldFederationsMutex);
    _heldFederations.push_front(0);
...
iMeshFederation*
cPathEngine::buildMeshFederation_TilingOnly(const cHorizontalRange& worldRange, long tileSize, long overlap)
{
