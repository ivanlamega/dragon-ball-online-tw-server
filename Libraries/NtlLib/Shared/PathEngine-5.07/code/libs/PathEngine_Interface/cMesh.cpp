...
//    _namedObstacleHeldShapes.push_back(agentC->getShapeHandle());
...
//            iAgent* result = placeAgent(_namedObstacleHeldShapes[i].get(), p);
            iAgent* result = placeAgent(_namedObstacleShapes[i], p);
...
{
    cMutexLock lock(_meshMutex);
    return static_cast<long>(_namedObstacleIDs.size());
}
void
...
//    agent = placeAgent(_namedObstacleHeldShapes[index].get(), p);
...
cPosition
cMesh::positionFor3DPoint_ExcludeTerrain(const long* point, long numberOfTerrainLayers) const
{
    cMutexLock lock(_meshMutex);
    tPoint p(point[0], point[1], WORLD_RANGE);
    float bestHeight;
...
//        ApproximatePointsAlongLine(sectionLine, static_cast<float>(distanceAlong.asDistance(sectionLine)), exactlyOnLine, p1, p2);
        {
            ...
            long iv = distanceAlong.multiplyLongByThis(segmentInterpolator.interpolatorMax());
...
long
cMesh::lookup3DFaceInSectionWithCoords(long sectionID, long* horizontalCoords, float* verticalCoords) const
{
    cMutexLock lock(_meshMutex);
    if(sectionID != -1 && !_sectionIDs)
    {
    ...
    {
        long segments = _partitioning->queryFacesPotentiallyStabbed(p0, &_partitioningResultsBuffer.front());
        long i;
        for(i = 0; i < segments; i++)
        {
            long f3DIndex = _partitioningResultsBuffer[i];
