...
Face3DForNoEdgesCase(tFace f2D, tMesh_3D& mesh3D)
{
    ...
    long startElementIndex = e2D.vertex()->getElementOn3DIndex();
    ...
    {
        long spinCount = SpinCountAroundVertex(e2D);
...
NextAround(
        ...
        const std::vector<long>& edgeBeforeLookup,
        tMesh_3D& mesh3D,
        long& edgeIndex
        )
{
  ...
  // interior edge
    ...
    long e3DIndex = -1 - edgeIndex;
    long edgeBeforeIndex = edgeBeforeLookup[e3DIndex];
        ...
        )
{
    ...
    std::vector<long>& edgeBeforeLookup = buffer;
...
// store this in vectors organised per boundary edge
...

    {
        ...
        {
            long startElementIndex = tMesh::beginEdge(f2D).vertex()->getElementOn3DIndex();
...
// generate the faces inside the boundary with a flood fill operation
  ...
  // (first face is generated outside the fill loop)
    ...
    while(!open.empty())
    {
        ...
        std::vector<tMesh_TranslationBSP::tEdgeData> edgeData;
        long startEdgeIndex = -1 - ePopped->_e3D.twin().index();
        long edgeIndex = startEdgeIndex;
        do
        {
            edgeData.push_back(EdgeDataFor(boundaryEdges, mesh3D, edgeIndex));
            NextAround(boundaryEdges, edgesBefore, edgesAfter, edgeBeforeLookup, mesh3D, edgeIndex);
