                        ...
                        ) const
{
    ...
    if(_shapeSet[i].lineCollides2D(segmentLineReversed.start(), segmentLineReversed.axis(), collidingLineIndex))
    {
      // get distance to exit
        tLine intersectLine = _shapeSet[i].getBoundaryLine(collidingLineIndex);
...
//void
//cShapesCostCalculator::swapOut(long index)
//{
...
// costs for shapes entered partway along the segment
    ...
    do
    {
        ...
        {
            long shapeIndex = _shapeSet.indexForShapeInFace(f.index(), i);
            ...
            long collidingLineIndex;
            if(!_shapeSet[shapeIndex].lineCollides2D(segmentLine.start(), segmentLine.axis(), collidingLineIndex))
            {
                continue;
            }
            
            tLine intersectLine = _shapeSet[shapeIndex].getBoundaryLine(collidingLineIndex);            
