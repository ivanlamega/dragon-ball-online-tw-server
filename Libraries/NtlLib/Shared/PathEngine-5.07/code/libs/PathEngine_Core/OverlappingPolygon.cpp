...
#include "libs/Geometry/interface/Polygon.h"
...
static bool
FaceIsCompletelyContained(const tPoint& centre, const std::vector<tPoint>& polygon, tFace f)
{
    tEdge e = tMesh::beginEdge(f);
    do
    {
        if(!PointInPolygon(SizeL(polygon), &polygon[0], GetVertexPoint(e)))
        {
            return false;
        }
    }
...
// get circuit parts and completely contained external faces
    ...
    do
    {
        if(f->isExternal())
        {
            if(FaceIsCompletelyContained(centre, polygon, f))
            {
        ...
        )
{
    ...
    return CircuitsForInfiniteCylinder(mesh, centre, polygon, containedExternalFaces);
}
