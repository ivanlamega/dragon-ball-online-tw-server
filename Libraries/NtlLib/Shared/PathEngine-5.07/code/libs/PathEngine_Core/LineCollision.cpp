    ...
    tFace startF, const tLine& testLine, tFace& endF)
{
    ...
    do
    {
        ...
        for(i = 0; i < elementLookup[currentF.index()].size(); i++)
        {
            ...
            long faceIndexAlongEl = elementLookup[currentF.index()][i].faceIndex;
            ...
            if( (faceIndexAlongEl > 0 && el.getEdgeCrossed_DoesntEnter(faceIndexAlongEl - 1) == edgeCrossedIn)
                ||
                (faceIndexAlongEl + 1 < el.facesCrossed() && el.getEdgeCrossed_DoesntEnter(faceIndexAlongEl).twin() == edgeCrossedIn)
                )
            {
          ...
          // testLine crosses el in 2d, and this occurs in or after the current face
            ...
            if( (faceIndexAlongEl > 0 && el.getEdgeCrossed_DoesntEnter(faceIndexAlongEl - 1).twin() == edgeCrossedOut)
                ||
                (faceIndexAlongEl + 1 < el.facesCrossed() && el.getEdgeCrossed_DoesntEnter(faceIndexAlongEl) == edgeCrossedOut)
                )
            {
            ...
            const cElementInFaceEntry& e, bool atStart, bool atEnd)
{
    ...
    if(e.element->getFaceCrossed(indexForFace) != intersectionF)
    {
            ...
            const cElementInFaceEntry& e)
{
    ...
    if(e.element->getFaceCrossed(0) != intersectionF)
    {
        // intersection did not resolve to the same piece of overlapping geometry as the intersected element
        return;
    }
        ...
        eSide bias)
{
    ...
    while(1)
    {
        ...
        if(info.element && info.element->getFaceCrossed(info.faceIndex) == currentF)
        {
            return;
        }
        tEdge edgeCrossed;
    ...
    cElementCollisionInfo& info)
{
    ...
    do
    {
        ...
        if(info.element && info.element->getFaceCrossed(info.faceIndex) == endF)
        {
            return;
        }
    }
    ...
    cElementCollisionInfo& info)
{
    ...
    if(info.element)
    {
        endF = info.element->getFaceCrossed(info.faceIndex);
    }
    else
    if(endF->isExternal())
    {
            ...
            )
{
    ...
    if(_crossingFound)
    {
        ...
        long comparison = tMultiplier::compare(firstMultiplied, candidateMultiplied);
        if(comparison == 1)
        {
        // after current first
            return;
    ...
    if(crossedElement.getFaceCrossed(indexForFace) != intersectionF)
    {
        // intersection did not resolve to the same piece of overlapping geometry as the intersected element
        return;
    }
            ...
            )
{
    ...
    if(crossedElement.getFaceCrossed(0) != intersectionF)
    {
        // intersection did not resolve to the same piece of overlapping geometry as the intersected element
        return;
    }
    ...
    for(i = elements.begin(); i != elements.end(); ++i)
    {
        ...
        if(i->element->lineEntersInFaceOrZeroLength(i->faceIndex, reversedLine, atStart, atEnd, intersect))
        {
            ...
            if(CompareFractions(startN, startD, n, d) == 1)
            {
                if(collides)
                {
                    long comparison = CompareFractions(firstN, firstD, n, d);
