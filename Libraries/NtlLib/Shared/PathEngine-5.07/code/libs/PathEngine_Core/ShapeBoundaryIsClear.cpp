        ...
        )
{
    ...
    tLine line(shape.centre().p, shape[0]);
    f = FaceAtEndOfLine<tMesh>(f, line);
    assertD(f.valid());

    if(basePointCollision.testPointCollision(qc, f, shape[0]))
    {
        return false;
    }
    if(PointCollision_AgainstShapeSet_IgnoringOneShape(shapeSet, cInternalPosition(f, shape[0]), indexForShapeToTest))
    {
        return false;
    }
    ...
    {
        tLine line = shape.getBoundaryLine(i);
        tFace endF;
