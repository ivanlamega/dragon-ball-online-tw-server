...
cQueryContext::cQueryContext(tMesh& mesh, long numberOfInternalFaces, bool meshHasInternalVertices, cAStarHeapPool& heapPool) :
 _mesh(mesh),
 ...
 _heapPool(heapPool)
{
    ...
    long facesSize = _mesh.faces() * sizeof(tFace);
...
void
cQueryContext::addElementCountRequirement(long elementCount)
{
...
cFlagVector&
cQueryContext::lockFlagVector(long sizeRequired)
{
    assertD(insideQuery());
    assertD(!_flagVectorLocked);
...
char*
cQueryContext::lockGeneralPurposeBuffer(long sizeRequired)
{
    assertD(insideQuery());
    assertD(!_generalPurposeBufferLocked);
    assertD(sizeRequired >= 0);
