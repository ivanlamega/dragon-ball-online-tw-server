...
bool
LineCollidesWithMesh(const nLineCollidesWithTriangle::cLinePreprocess& line, const std::vector<nLineCollidesWithTriangle::cTriPreprocess>& meshPreprocess, long& faceIndex)
{
    size_t i;
...
bool
RayCollidesWithMesh(const nLineCollidesWithTriangle::cLinePreprocess& line, const std::vector<nLineCollidesWithTriangle::cTriPreprocess>& meshPreprocess, long& faceIndex, float& fractionAlong)
{
    size_t i;
    faceIndex = -1;
    for(i = 0; i < meshPreprocess.size(); i++)
                         ...
                         std::vector<long>& appendTo)
{
    ...
    for(i = 0; i < meshPreprocess.size(); i++)
    {
        if(nLineCollidesWithTriangle::lineCollidesWithTriangle(meshPreprocess[i], line))
        {
            appendTo.push_back(static_cast<long>(i));
        }
    }
}
