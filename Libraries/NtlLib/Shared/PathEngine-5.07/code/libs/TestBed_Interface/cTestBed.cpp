...
cTestBed::drawRangeBounds(const cHorizontalRange& bounds)
{
    ...
    long x = bounds.maxX - bounds.minX;
    if(x < 2)
    {
    ...
    x /= 2;
    x += bounds.minX;
    long y = bounds.maxY - bounds.minY;
    if(y < 2)
    {
    ...
    std::vector<tPoint> polygon(4);
    polygon[0] = tPoint(bounds.minX, bounds.minY, WORLD_RANGE);
    polygon[1] = tPoint(bounds.minX, bounds.maxY, WORLD_RANGE);
    polygon[2] = tPoint(bounds.maxX, bounds.maxY, WORLD_RANGE);
    polygon[3] = tPoint(bounds.maxX, bounds.minY, WORLD_RANGE);
    std::list<tFace> containedFaces;
    cHandle<tMeshCircuits> boundsCircuits = CircuitsForInfiniteCylinder(
            _mesh->ref2D(),
            centre,
            polygon,
            containedFaces
