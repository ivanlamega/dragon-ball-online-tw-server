...
static void
CopyFaceAndConnect(
        tMesh_3D& copyFrom, long index, tMesh_3D& copyTo,
        const std::vector<long>& faceRemap
        )
{
    tFace_3D f = copyFrom.faceAt(index);
    long edges = CountEdgesAround(tMesh_3D::beginEdge(f));
    ...
    do
    {
        ...
        if(!twinF->isExternal())
        {
            long twinIndex = twinF.index();
            long remappedTwinIndex = faceRemap[twinIndex];
...
//  and connect edges where the twins face has already been copied
    ...
    {
        for(long j = 0; j != subsetsLookup.subVectorSize(i); j += 2)
        {
    ...
    case bMeshElement::EDGE:
        {
            ...
            long remappedFIndex = faceRemap[e.face().index()];
            if(remappedFIndex == -1)
            {
                e = e.twin();
