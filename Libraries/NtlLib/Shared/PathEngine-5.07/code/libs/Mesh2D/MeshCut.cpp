...
long
cMeshCut::indexElements(long startIndex) const
{
    assertD(_startElement);
...
{
    if(e1->getFaceCrossed(0).index() < e2->getFaceCrossed(0).index())
    {
        return true;
    }
    if(e1->getFaceCrossed(0).index() > e2->getFaceCrossed(0).index())
...
cMeshCut::compareIntersections(const tIntersection& lhs, const tIntersection& rhs)
{
    ...
    long comparison = CompareFractions(n1, d1, n2, d2);
    if(comparison != 0)
    {
        return comparison;
    }
...
cMeshCut::compareEntries(const cCircuitElement* lhs, const cCircuitElement* rhs)
{
    ...
    long comparison = compareEntryOrExitEdges(lhs_ee, rhs_ee);
    if(comparison != 0)
    {
        return comparison;
    }
...
cMeshCut::compareCorners(const cCircuitElement* lhs, const cCircuitElement* rhs)
{
    ...
    lhs_face = lhs->getFaceCrossed(0);
    rhs_face = rhs->getFaceCrossed(0);
...
{
    assertD(_startElement);
    return _startElement->getExternalFaceForEntry();
}
...
{
    assertD(_startElement);
    return _startElement->getPrev()->getExternalFaceForExit();
}
...
{
    assertD(_startElement);
    return _startElement->getFaceCrossed(0);
}
...
cMeshCut::remap(const iMeshTranslator<tMesh>& translator)
{
    ...
    if(!e->enters())
    {
        ...
        cMeshElement<tMesh> el = ElementForPointInOverlappingFace<tMesh, tIntersection>(_startElement->getFaceCrossed(0), intersection);
        ...
        do
        {
            ...
            startFace = e->getLastFaceCrossed();
            e = e->getNext();
        }
    ...
    else
    {
        ...
        {
            tFace startFace = e->getLastFaceCrossed();
            e = e->getNext();
            e->remakeTraversal(startFace);
        }
    }
...
bool
cMeshCut::isSmallConvex(cFacesContainedByBoundary& fcbb, long maximumCircumference) const
{
    if(!isInternalLoop())
    {
        return false;
    ...
    {
        long i;
        for(i = 0; i < e->getEdgesCrossed(); i++)
        {
            fcbb.edgeCrossed(e->getEdgeCrossed(i));
        }
        e = e->getNext();
    }
    while(e != _startElement);
...
CompareCutSequences(const tMeshCutSequence& lhs, const tMeshCutSequence& rhs)
{
    ...
    {
        long comparison = cMeshCut::compare(**i, **j);
        if(comparison != 0)
        {
            return comparison;
        }
...
void
GetIntersectedExternalFaces(long facesInMesh, tMeshCircuits& circuits,
...
bool
CircuitIsSmallConvex(cFacesContainedByBoundary& fcbb, const tMeshCutSequence& circuit, long maximumCircumference)
{
    return circuit._list.size() == 1 && circuit.front()->isSmallConvex(fcbb, maximumCircumference);
}

void
SplitSmallConvex(long edgesInMesh, tMeshCircuits& circuits, long maximumCircumference, tMeshCircuits& appendTo)
{
...
tFace
cMeshCut::iterateOverFacesCrossed(const cCircuitElement*& e, long& faceIndex) const
{
    tFace result = e->getFaceCrossed(faceIndex);
    faceIndex++;
        ...
        for(j = sequence.begin(); j != sequence.end(); ++j)
        {
            ...
            do
            {
                ...
                for(k = 0; k < e->facesCrossed(); k++)
                {
                    if(k)
                    {
                        tEdge crossed = e->getEdgeCrossed(k - 1);
...
long
IndexElements(const tMeshCutSequence& sequence, long startIndex)
{
    long currentIndex = startIndex;
    tMeshCutSequence::const_iterator j;
    for(j = sequence.begin(); j != sequence.end(); ++j)
...
long
IndexElements(const tMeshCircuits& circuits, long startIndex)
{
    long currentIndex = startIndex;
    tMeshCircuits::const_iterator j;
    for(j = circuits.begin(); j != circuits.end(); ++j)
...
{
    os << static_cast<long>(circuits.size());
    tMeshCircuits::const_iterator i;
    for(i = circuits.begin(); i != circuits.end(); ++i)
    {
        os << static_cast<long>((*i)->size());
        tMeshCutSequence::const_iterator j;
        for(j = (*i)->begin(); j != (*i)->end(); ++j)
        {
