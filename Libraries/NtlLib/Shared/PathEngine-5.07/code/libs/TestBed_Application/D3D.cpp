...
HRESULT
cD3D::init(HWND hWnd, long sizeX, long sizeY)
{
    _sizeX = sizeX;
    _sizeY = sizeY;
    ...
    if(0 == (_pD3D = Direct3DCreate8(D3D_SDK_VERSION)))
    {
        MessageBox(0, "Direct3dCreate8() failed", "Error in renderer.", MB_OK);
        return E_FAIL;
    }
    ...
    HRESULT result;
    result = _pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd,
    ...
    ZeroMemory(&_mtrl, sizeof(D3DMATERIAL8));
    ...
    ZeroMemory(&_light, sizeof(D3DLIGHT8));
...
cD3D::setAmbient()
{
    ...
    value <<= 8;
    value += static_cast<unsigned long>(_r * 0xff * _ambientPercent);
    value <<= 8;
    value += static_cast<unsigned long>(_g * 0xff * _ambientPercent);
    value <<= 8;
    value += static_cast<unsigned long>(_b * 0xff * _ambientPercent);
    HRESULT result;
...
cD3D::setRenderState(eRenderState state)
{
    ...
    case RS_LINE:
        ...
        setAmbient();
        result=_pd3dDevice->SetVertexShader(D3DFVF_LINEVERTEX);
        assertD(result==D3D_OK);
    ...
    case RS_TEXT:
        _ambientPercent = 1.0f;
        setAmbient();
        result=_pd3dDevice->SetVertexShader(D3DFVF_TEXTVERTEX);
        assertD(result==D3D_OK);
    ...
    case RS_POLYGON:
        ...
        setAmbient();
        result=_pd3dDevice->SetVertexShader(D3DFVF_POLYGONVERTEX);
        assertD(result==D3D_OK);
...
void
cD3D::setLoadedImageRenderState(long index)
{
    HRESULT result;
    _ambientPercent = 1.0f;
    setAmbient();
...
void
cD3D::setStreamSource(IDirect3DVertexBuffer8* source, UINT size)
{
    if(source != _currentstreamsource)
    {
        HRESULT result;
        result = _pd3dDevice->SetStreamSource(0, source, size);
        assertD(result == D3D_OK);
...
void
cD3D::setCamera2D(long scale, long x, long y, bool zIsIgnored)
{
...
//    }
    ...
    long result = static_cast<long>(_loadedImages.size());
