...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        cD3D::refInstance().init(hWnd, gConfig.sizeX, gConfig.sizeY);
        InitDSound(hWnd);
        break;
    ...
    case WM_MOUSEMOVE:
        cControlInterface::refInstance().setMousePosition(
                                        LOWORD(lParam),
                                        gConfig.sizeY - 1 - static_cast<long>(HIWORD(lParam)));
        break;

    case WM_KEYDOWN:
        nKeyBuffer::keyPressed(wParam);
        break;

    case WM_KEYUP:
        nKeyBuffer::keyReleased(wParam);
        break;
...
CreateMainWindow(HINSTANCE hInstance, int width, int height, int showCommand)
{
    ...
    HWND hWnd = CreateWindow("ClassName", "Windowed testbed", 
...
{
    if(stricmp(arg,"notdinput")==0)
    {
        config.useDInput = false;
        return true;
    }
    if(strnicmp(arg, "windowsize=", 11) == 0)
    {
        char* ptr;
    ...
    LPTSTR cmdLine, int showCommand)
{
    ...
    ClientEntryPoint(&cOSInterface::refInstance(), argsLeft, nextArgV);
