...
//== add entry methods
    ...
    long addVertAtEnd(const tVertexData& value)
    {
        long result;
...
//====
    ...
    append(cVectorMesh<F2,E2,V2>& rhs)
    {
        ...
        _edges += rhs._edges;
        _verts += rhs._verts;
        long faceStart = faceSize();
        long edgeStart = edgeSize();
        long vertStart = vertSize();
    ...
    split(tVertexPredicate vertexPredicate, tThis& splitTo)
    {
        ...
        std::vector<long> remaps(faceSize() + edgeSize() + vertSize(), -1);
        long* faceRemaps = &remaps[0];
        long* edgeRemaps = &remaps[faceSize()];
        long* vertRemaps = &remaps[faceSize() + edgeSize()];
        vertex v = beginVertex();
        do
        {
