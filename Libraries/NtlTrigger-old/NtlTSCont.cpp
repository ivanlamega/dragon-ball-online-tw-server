...
bool CNtlTSCont::AttachChildEntity( CNtlTSEntity* pEntity )
{
	if ( 0 == pEntity )
	{
		CNtlTSLog::Log( "Can not find the entity. ParamInfo[%s]. [%s]", pEntity ? pEntity->GetClassName() : "NULL", TS_CODE_TRACE() );
		return false;
	}
...
bool CNtlTSCont::AttachPrevLink( NTL_TS_TC_ID tcId )
{
	...
	{
		CNtlTSLog::Log( "Can not attach the invalid container. ParamInfo[%d]. [%s]", tcId, TS_CODE_TRACE() );
		return false;
	}
...
void CNtlTSCont::DetachPrevLink( NTL_TS_TC_ID tcId )
{
	if ( NTL_TS_TC_ID_INVALID == tcId )
	{
		CNtlTSLog::Log( "Can not detach the invalid container. ParamInfo[%d]. [%s]", tcId, TS_CODE_TRACE() );
		return;
	}
	...
	{
		CNtlTSLog::Log( "Can not find the container. ParamInfo[%d]. [%s]", tcId, TS_CODE_TRACE() );
	}
}
...
void CNtlTSCont::AttachChildForScript( CNtlTSScrObject* pChild )
{
	...
	{
		CNtlTSLog::Log( "Can not do type cast from CNtlTSScrObject to CNtlTSEntity. ParamInfo[%s]. [%s]", pChild ? pChild->GetClassName() : "NULL", TS_CODE_TRACE() );
	}
}
...
bool CNtlTSCont::ParsingLinkInfo( const std::string& strContents )
{
	...
	for ( ; cit != strContents.end(); ++cit )
	{
		...
		else if ( '0' > *cit || '9' < *cit )
		{
			...
			CNtlTSLog::Log( "The link info must have to be NUMERIC" );
