...
CNtlTSUIObject* CNtlTSUIFactory::CreateObj( const std::string& strName )
{
	...
	{
		CNtlTSLog::Log( "Can not find the UI object type. Info[%s]. [%s]", strName.c_str(), TS_CODE_TRACE() );
	}
...
void CNtlTSUIFactory::DeleteObj( CNtlTSUIObject*& pObj )
{
	if ( pObj )
	{
		...
		{
			CNtlTSLog::Log( "Don't find the UI object(%s).[%s]", pObj->GetClassName(), TS_CODE_TRACE() );
		}
	}
}
