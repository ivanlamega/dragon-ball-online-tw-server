...
CNtlTSScrObject* CNtlTSEntityFactory::CreateObj( const std::string& strName )
{
	...
	{
		CNtlTSLog::Log( "Can not find the class type. ParamInfo[%s]. [%s]", strName.c_str(), TS_CODE_TRACE() );
	}
...
void CNtlTSEntityFactory::DeleteObj( CNtlTSScrObject*& pObj )
{
	if ( pObj )
	{
		...
		for ( ; itObj != defObjList.end(); ++itObj )
		{
			...
			{
				CNtlTSLog::Log( "The class type is not exist. ParamInfo[%s]. [%s]", (*itObj)->GetClassName(), TS_CODE_TRACE() );
			}
		}
