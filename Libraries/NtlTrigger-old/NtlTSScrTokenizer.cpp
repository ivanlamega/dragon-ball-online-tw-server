...
bool CNtlTSScrTokenizer::Load( const std::string& strFileName, const char* pBuff, int nSize )
{
	...
	else
	{
		...
		{
			CNtlTSLog::Log( "Can not open the file. Info[%s]. [%s]", strFileName.c_str(), TS_CODE_TRACE() );
			return false;
		}
		...
		if ( 0 == m_pFileBuffer )
		{
			CNtlTSLog::Log( "Allocating a memory is failed. Info[%d]. [%s]", m_nFileSize*sizeof(char), TS_CODE_TRACE() );
			return false;
		}
	...
	while ( nCurPos < m_nFileSize )
	{
		...
		else
		{
			...
			if ( '"' == m_pFileBuffer[nTempPos] )
			{
				...
				{
					CNtlTSLog::Log( "Parsing the string is failed. Info[%d]. [%s]", nTempPos, TS_CODE_TRACE() );
					return false;
				}
				...
				if ( 0 == pTemp )
				{
					CNtlTSLog::Log( "Allocating a memory is failed. Info[%d]. [%s]", (nNumChars+1)*sizeof(char), TS_CODE_TRACE() );
					return false;
				}
				pTemp[nNumChars] = 0;
